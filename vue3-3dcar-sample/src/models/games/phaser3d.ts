...
export class Phaser3D extends Phaser.Events.EventEmitter {
  ...
  composer: any;
make: {
box: (config: any) => THREE.Mesh<any, any>; cone: (config: any) => THREE.Mesh<any, any>; circle: (config: any) => THREE.Mesh<any, any>; cylinder: (config: any) => THREE.Mesh<any, any>; dodecahedron: (config: any) => THREE.Mesh<any, any>;
//extrude: (config) => this.makeExtrude(config),
icosahedron: (config: any) => THREE.Mesh<any, any>;
//lathe: (config) => this.makeLathe(config),
plane: (config: any) => THREE.Mesh<any, any>;
//parametric: (config) => this.makeParametric(config),
ring: (config: any) => THREE.Mesh<any, any>; sphere: (config: any) => THREE.Mesh<any, any>;
//shape: (config) => this.makeShape(config),
//text: (config) => this.makeText(config),
//tube: (config) => this.makeTube(config),
octahedron: (config: any) => THREE.Mesh<any, any>; polyhedron: (config: any) => THREE.Mesh<any, any>; tetrahedron: (config: any) => THREE.Mesh<any, any>; torus: (config: any) => THREE.Mesh<any, any>; torusKnot: (config: any) => THREE.Mesh<any, any>;
};
  ...
  constructor(
    ...
    this.renderer = new THREE.WebGLRenderer({
      ...
      context: phaserScene.sys.game.context as WebGLRenderingContext,
      antialias: true,
    });
  ...
  enableGamma(input = true, output = true) {
    (this.renderer as any).gammaInput = input;
    (this.renderer as any).gammaOutput = output;
    return this;
  }
  ...
  //       .setPath(path)
  //       .load([right, left, up, down, back, front]);
  //   }
  ...
  createMaterial(texture: any, color: any, material: any) {
    if (material && !Phaser.Utils.Objects.IsPlainObject(material)) {
      ...
      if (texture) {
        config.map =
          typeof texture === "string" ? this.getTexture(texture) : texture;
      }
  ...
  //       geometry,
  //       this.createMaterial(texture, color, material),
  //       x,
  //       y,
  //       z
  //     );
  ...
  //     data.geometry,
  //     this.createMaterial(data.texture, data.color, data.material)
  //   );
  // }

  // addPoints(config: any) {
