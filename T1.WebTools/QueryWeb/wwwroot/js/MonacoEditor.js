...
window.StaticMonacoEditor = {
    ...
    getCurrentLineInfo: function (editorRef) {
        ...
        const prevText = this.getPrevCursorText(editorRef);
        return { prev, line, after, prevText };
    },
    getPrevCursorText: function (editorRef) {
        const editor = getEditor(editorRef);
        const currentPosition = editor.getPosition();
        // 計算最先開頭的位置
        let startPosition = new monaco.Position(currentPosition.lineNumber - 1, 1);
        ...
        if( startPosition.lineNumber < 1) {
            startPosition.lineNumber = currentPosition.lineNumber;
        }
        const range = new monaco.Range(startPosition.lineNumber, 1, 
