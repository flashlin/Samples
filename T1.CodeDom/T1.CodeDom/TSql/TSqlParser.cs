...
namespace T1.CodeDom.TSql
{
    public class TSqlParser : PrattParser
    {
        public TSqlParser(IScanner scanner) : base(scanner)
        {
            ...
            Register(SqlToken.UnPivot, new PivotParselet());
            Register(SqlToken.Variable, new VariableParselet());
            Register(SqlToken.Values, new ValuesParselet());
            Register(SqlToken.With, new WithParselet());
            Register(SqlToken.While, new WhileParselet());
            Register(SqlToken.WaitFor, new WaitForParselet());
            ...
            Register(SqlToken.NEWID, new CallFuncParselet());
            Register(SqlToken.NODES, new CallFuncParselet(1, 1));
        ...
        protected override (TextSpan infixToken, int consumeIndex) PeekToken()
        {
            ...
            do
            {
                ...
                if (infixToken.Type == SqlToken.MultiComment.ToString() ||
                    infixToken.Type == SqlToken.SingleComment.ToString())
                {
                    index = consumeIndex;
                    continue;
                }
        ...
        protected override IPrefixParselet CodeSpecPrefix(TextSpan token)
        {
            ...
            catch (KeyNotFoundException)
            {
                ...
                throw new ParseException(
                    $"Not found SqlType.{token.Type} '{tokenStr}' in PrefixParselets map.\r\n{helpMessage}");
            }
        }
    }
}