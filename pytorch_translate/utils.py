...
class Vocabulary:
    ...
    def build_vocabulary(self, sentence_list):
        ...
        frequencies = dict(
            sorted(frequencies.items(), key=lambda x: -x[1])[:self.max_size - idx])  # idx =4 for pad, start, end , unk
...
def get_train_loader(dataset, batch_size, num_workers=0, shuffle=True,
                     pin_memory=True):  # increase num_workers according to CPU
    ...
    loader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers,
                        ...
                        collate_fn=MyCollate(pad_idx=pad_idx))  # MyCollate class runs __call__ method by default
    return loader
