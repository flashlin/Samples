...
> 入口：`SqlParser.Parse()` dispatch 13 種頂層語句（WITH CTE / CREATE TABLE / CREATE VIEW / SELECT / INSERT / UPDATE / DELETE / MERGE / TRUNCATE TABLE / DROP / ALTER TABLE / EXEC sp_addextendedproperty / SET）。
...
- [~] `DROP ...`（支援 `DROP {TABLE|VIEW|PROCEDURE|FUNCTION|INDEX|TRIGGER|SCHEMA|DATABASE|SEQUENCE|TYPE} [IF EXISTS] name1[, ...]`；`SqlDropStatement` + `SqlDropObjectType` enum。未做 `DROP INDEX idx ON table` 變體）
...
- [ ] `CREATE PROCEDURE` / `FUNCTION` / `INDEX` / `TRIGGER` / `SCHEMA` / `DATABASE`
- [ ] `DECLARE`
- [ ] `IF / ELSE`
- [ ] `WHILE`
- [ ] `BEGIN ... END`
...
1. 🟢 DDL（DROP/TRUNCATE/ALTER TABLE/CREATE VIEW 已完成）：`CREATE INDEX`、`CREATE PROC|FUNCTION`、`DROP INDEX ... ON table` 變體、ALTER TABLE 第二階段（見 §1.5）
