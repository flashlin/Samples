...
> 入口：`SqlParser.Parse()` dispatch ~20 種頂層語句（WITH CTE / CREATE TABLE|VIEW|INDEX|PROCEDURE / SELECT / INSERT / UPDATE / DELETE / MERGE / TRUNCATE / DROP / ALTER TABLE / EXEC sp_addextendedproperty / EXEC proc / DECLARE / BEGIN…END / IF / WHILE / SET）。
...
- [ ] `CREATE FUNCTION` / `TRIGGER` / `SCHEMA` / `DATABASE`
...
1. 🟢 控制流程續攻（DECLARE/IF/WHILE/BEGIN…END 已完成）：`BEGIN TRY…END TRY / BEGIN CATCH`、`BEGIN/COMMIT/ROLLBACK TRANSACTION`、`BREAK`/`CONTINUE`/`RETURN`、`PRINT`/`THROW`/`RAISERROR`、`DECLARE @t TABLE`
2. 🟢 DDL（CREATE PROCEDURE 已完成）：`CREATE FUNCTION|TRIGGER`、ALTER TABLE 第二階段（見 §1.5）
