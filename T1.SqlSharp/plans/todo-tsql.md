...
> 最後驗證：2026-06-21（依 `T1.SqlSharp/ParserLit/SqlParser.cs`、`LinqParser.cs` 與測試實際比對；356 測試全綠）。
> 入口：`SqlParser.Parse()` dispatch ~34 種頂層語句（WITH CTE / CREATE TABLE|VIEW|INDEX|PROCEDURE|FUNCTION|TRIGGER|SCHEMA|DATABASE / SELECT / INSERT / UPDATE / DELETE / MERGE / TRUNCATE / DROP / ALTER TABLE / EXEC sp_addextendedproperty / EXEC proc / DECLARE / BEGIN TRY…CATCH / TRANSACTION（BEGIN|COMMIT|ROLLBACK|SAVE）/ BEGIN…END / IF / WHILE / RETURN / PRINT / THROW / RAISERROR / BREAK / CONTINUE / USE / GO / GRANT|REVOKE|DENY / SET）。
...
- [~] `ALTER TABLE`（ADD 欄位（多）/ ADD CONSTRAINT（含 `WITH CHECK/NOCHECK` 前綴）/ DROP COLUMN（多）/ DROP CONSTRAINT（多）/ ALTER COLUMN / `{ENABLE\|DISABLE} TRIGGER {ALL\|names}` / `{CHECK\|NOCHECK} CONSTRAINT {ALL\|names}`，細目見 §1.5。剩 ADD 混合欄位+約束）；`ALTER VIEW/PROCEDURE/FUNCTION/TRIGGER` 已支援（見對應 CREATE 列）
...
- [~] `CREATE VIEW` / `ALTER VIEW`（`{CREATE [OR ALTER]|ALTER} VIEW v [(cols)] AS <query> [WITH CHECK OPTION]`；body 走 `Parse_CteBodyStatement` 故支援 CTE/SELECT。`SqlCreateViewStatement`（`IsAlter` 旗標）。未做 `WITH SCHEMABINDING/ENCRYPTION`）
...
- [~] `CREATE PROCEDURE` / `ALTER PROCEDURE`（`{CREATE [OR ALTER]|ALTER} {PROCEDURE|PROC} name [([@p type [(size)] [= default] [OUTPUT]] , ...)] AS <body>`；body 走 `Parse()`（單一語句／`BEGIN…END`）。`SqlCreateProcedureStatement`（`IsAlter` 旗標）+ `SqlProcedureParameter`。未做 `WITH` 選項、`FOR REPLICATION`、無 BEGIN 的多裸語句 body）
- [~] `CREATE FUNCTION` / `ALTER FUNCTION`（scalar：`RETURNS type[(size)] AS <body>`；inline TVF：`RETURNS TABLE AS RETURN (select)`；multi-statement TVF：`RETURNS @t TABLE (col defs) AS BEGIN…RETURN END`（`ReturnTableVariable`/`ReturnTableColumns`，欄位重用 `ParseColumnDefinition`）。`SqlCreateFunctionStatement`（`IsAlter` 旗標），body 走 `Parse()`、return clause 抽 `ParseFunctionReturnClause`。未做 `WITH SCHEMABINDING`/`RETURNS NULL ON NULL INPUT` 等選項）
- [~] `CREATE TRIGGER` / `ALTER TRIGGER`（`SqlCreateTriggerStatement`（`IsAlter` 旗標）+ `SqlTriggerTiming`/`SqlTriggerEvent` enum；`{CREATE [OR ALTER]|ALTER} TRIGGER name ON target {FOR|AFTER|INSTEAD OF} {INSERT|UPDATE|DELETE}[, ...] AS <body>`，body 重用 `Parse()`。未做 `WITH` 選項、`DDL/LOGON` trigger、`FOR EACH ROW`）
...
- [~] `DECLARE`（`DECLARE @v type [(size)] [= value] [, ...]`；`@t TABLE (col defs)` 表變數（`IsTable`/`TableColumns`，欄位重用共用 `Parse_ParenthesizedColumnDefinitions`）；`SqlDeclareStatement` + `SqlVariableDeclaration`。未做 `CURSOR`、表變數內 table 約束）
...
- [ ] ADD 同時混合欄位 + 約束（`ADD c INT, CONSTRAINT ...`）
...
1. 🟢 控制流程續攻（DECLARE（含 `@t TABLE`）/IF/WHILE/BEGIN…END/RETURN/TRY…CATCH/TRANSACTION/BREAK/CONTINUE/PRINT/THROW/RAISERROR 已完成）：`DECLARE @c CURSOR`、`BEGIN DISTRIBUTED TRANSACTION`、`;THROW` 分號語意
2. 🟢 DDL（CREATE+ALTER VIEW/PROC/FUNCTION（含 multi-statement TVF）/TRIGGER、SCHEMA/DATABASE、ALTER TABLE 第二階段已完成）：ADD 混合欄位+約束、各語句 `WITH` 選項（SCHEMABINDING/ENCRYPTION）、DATABASE/SCHEMA 進階選項
