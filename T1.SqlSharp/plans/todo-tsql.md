...
> 最後驗證：2026-06-21（依 `T1.SqlSharp/ParserLit/SqlParser.cs`、`LinqParser.cs` 與測試實際比對；366 測試全綠）。
...
1. 🟢 控制流程（DECLARE（含 `@t TABLE`/`@c CURSOR`）/IF/WHILE/BEGIN…END/RETURN/TRY…CATCH/TRANSACTION/BREAK/CONTINUE/PRINT/THROW/RAISERROR 已完成）：`BEGIN DISTRIBUTED TRANSACTION`、`;THROW` 分號語意、CURSOR 進階選項
