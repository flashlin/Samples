...
> 最後驗證：2026-06-21（依 `T1.SqlSharp/ParserLit/SqlParser.cs`、`LinqParser.cs` 與測試實際比對；604 測試全綠；全項目 [x],無 [~]/[ ] 殘留）。
...
- [x] `SET <option> {ON|OFF | value}`（session 選項：`SET NOCOUNT ON`/`XACT_ABORT OFF`、`SET IDENTITY_INSERT table ON`、取值型 `SET ROWCOUNT 100`/`SET DATEFORMAT mdy`/`SET LOCK_TIMEOUT 1000`；`SET TRANSACTION ISOLATION LEVEL {READ UNCOMMITTED|READ COMMITTED|REPEATABLE READ|SNAPSHOT|SERIALIZABLE}`，`ReadIsolationLevel` 處理 1–2 字 level）（`SqlSetOptionStatement { Option, Target, Value }`，`ParseSetOptionStatement` 排在變數賦值前、peek `=` 則 reset 落回 `SET @x = ...`；ON/OFF 用 `TryMatchOnOff`、其餘值走 `ParseArithmeticExpr().ToSql()`）
...
- [x] `DELETE`（parser 可解析：`[FROM] t` / 省略 FROM / 第二個 `FROM`+JOIN / `WHERE` / `TOP` / table hint / `OUTPUT` / CTE 前綴，細目見 §1.3。已大致完整）
...
- [x] `ALTER TABLE`（ADD 欄位（多）/ ADD CONSTRAINT（含 `WITH CHECK/NOCHECK` 前綴）/ DROP COLUMN（多）/ DROP CONSTRAINT（多）/ ALTER COLUMN / `{ENABLE\|DISABLE} TRIGGER {ALL\|names}` / `{CHECK\|NOCHECK} CONSTRAINT {ALL\|names}`，細目見 §1.5。含 ADD 混合欄位+約束）；`ALTER VIEW/PROCEDURE/FUNCTION/TRIGGER` 已支援（見對應 CREATE 列）
- [x] `DROP ...`（核心型別走 `SqlDropObjectType` enum：`TABLE|VIEW|PROCEDURE|FUNCTION|INDEX|TRIGGER|SCHEMA|DATABASE|SEQUENCE|TYPE|SYNONYM|LOGIN|USER|ROLE`；擴充型別走 `SqlDropStatement.TypeName` 字串（多字 via `MatchMultiWordDropType`）：`CERTIFICATE`/`ASSEMBLY`/`CREDENTIAL`/`AGGREGATE`/`RULE`/`DEFAULT`/`QUEUE`/`SERVICE`/`CONTRACT`/`ENDPOINT`/`STATISTICS`、`MESSAGE TYPE`/`MASTER KEY`/`SYMMETRIC KEY`/`PARTITION FUNCTION`/`PARTITION SCHEME`/`FULLTEXT INDEX (ON t)`/`FULLTEXT CATALOG`/`SECURITY POLICY`/`EXTERNAL TABLE`/`EXTERNAL DATA SOURCE`/`EXTERNAL FILE FORMAT`/`COLUMN MASTER KEY`/`COLUMN ENCRYPTION KEY`；`[IF EXISTS]` + 多名稱 + `DROP INDEX ix ON table`。舊式 `DROP INDEX table.idx`（含多組 `t1.ix1, t2.ix2`）以 qualified `Names` 表示）
- [x] `TRUNCATE TABLE`（`SqlTruncateTableStatement`）
- [x] `CREATE VIEW` / `ALTER VIEW`（`{CREATE [OR ALTER]|ALTER} VIEW v [(cols)] [WITH opt[, ...]] AS <query> [WITH CHECK OPTION]`；AS 後 body：leading `WITH` 走 `ParseWithCteStatement`（CTE body）、否則走 `Parse_CteBodyStatement`（SELECT/INSERT/UPDATE/DELETE/MERGE）；AS 前 `WITH opt`（SCHEMABINDING/ENCRYPTION…，含多字 `EXECUTE AS x`/`RETURNS NULL ON NULL INPUT`/`CALLED ON NULL INPUT`）入 `Options`（共用 `Parse_WithOptionList`/`Parse_WithOption`）。`SqlCreateViewStatement`（`IsAlter` 旗標）。）
- [x] `CREATE INDEX`（支援 `CREATE [UNIQUE] [CLUSTERED|NONCLUSTERED] [COLUMNSTORE] INDEX ix ON t [(col [ASC|DESC], ...)] [INCLUDE (cols)] [WHERE filter] [WITH (options)]`；含 clustered/nonclustered COLUMNSTORE（`IsColumnstore`，欄位可省）、`SPATIAL`（`IsSpatial`）；重用 `ParseColumnsAscDesc`。`SqlCreateIndexStatement`）
- [x] `CREATE PROCEDURE` / `ALTER PROCEDURE`（`{CREATE [OR ALTER]|ALTER} {PROCEDURE|PROC} name [([@p type [(size)] [= default] [OUTPUT]] , ...)] AS <body>`；AS 前 `WITH opt[, ...]`（ENCRYPTION/RECOMPILE…）入 `Options`；body 走 `ParseProcedureBody`：收集多語句直到 `GO`/EOF（T-SQL batch 語意），單句直接存、多句存為 `SqlBlockStatement{IsImplicit=true}`（ToSql 不加 BEGIN/END），故支援無 BEGIN 的多裸語句 body（如 `AS SET NOCOUNT ON SELECT 1`）。含多字 `EXECUTE AS {CALLER|OWNER|SELF}`。`SqlCreateProcedureStatement`（`IsAlter` 旗標）+ `SqlProcedureParameter`。未做 `FOR REPLICATION`）
- [x] `CREATE FUNCTION` / `ALTER FUNCTION`（scalar：`RETURNS type[(size)] AS <body>`；inline TVF：`RETURNS TABLE AS RETURN (select)`；multi-statement TVF：`RETURNS @t TABLE (col defs) AS BEGIN…RETURN END`（`ReturnTableVariable`/`ReturnTableColumns`，欄位重用 `ParseColumnDefinition`）。AS 前 `WITH opt[, ...]`（SCHEMABINDING…）入 `Options`。`SqlCreateFunctionStatement`（`IsAlter` 旗標），body 走 `Parse()`、return clause 抽 `ParseFunctionReturnClause`。含多字選項 `RETURNS NULL ON NULL INPUT`/`CALLED ON NULL INPUT`）
- [x] `CREATE TRIGGER` / `ALTER TRIGGER`（`SqlCreateTriggerStatement`（`IsAlter` 旗標）+ `SqlTriggerTiming`/`SqlTriggerEvent` enum；`{CREATE [OR ALTER]|ALTER} TRIGGER name ON target {FOR|AFTER|INSTEAD OF} {INSERT|UPDATE|DELETE}[, ...] AS <body>`；ON 後、timing 前 `WITH opt[, ...]`（ENCRYPTION…）入 `Options`；body 重用 `Parse()`。DDL/LOGON trigger：`ON {DATABASE|ALL SERVER}` + DDL 事件名（`DdlEvents` 字串清單，如 `CREATE_TABLE`/`LOGON`）已支援。`FOR EACH ROW` 為 MySQL/標準 SQL row-level trigger 語法，T-SQL 無此語法（T-SQL trigger 為 statement-level）→ N/A）
- [x] `CREATE SCHEMA`（`SqlCreateSchemaStatement`；`CREATE SCHEMA name [AUTHORIZATION owner] [<schema_element> ...]`，inline 物件走 `ParseSchemaElement`（`CREATE TABLE`/`CREATE VIEW`/`GRANT`/`REVOKE`/`DENY`）收集進 `Elements`）/ `CREATE DATABASE`（`SqlCreateDatabaseStatement`；`CREATE DATABASE name [ON [PRIMARY] (filespec)[, ...] [, FILEGROUP fg (filespec)[, ...]] ...] [LOG ON (filespec)[, ...]] [COLLATE x]`，filespec 走 `ReadFileSpecList` 收集 `(K = V[, ...])`，值走 `ReadFileSpecValue` 支援帶單位 `SIZE = 10MB`/`FILEGROWTH = 5MB`（number + 緊接 unit 合併）；`FILEGROUP fg (...)` 自訂檔案群組存 `FileGroups`（`SqlDatabaseFileGroup`）；`CONTAINMENT = {PARTIAL|NONE}`（`Containment`）、結尾 `WITH opt[, ...]`（`ReadDatabaseWithOptions` 處理 `K = V`/`K ON`/`K OFF`，如 `TRUSTWORTHY ON`、`DB_CHAINING OFF`，存 `Options`）)
- [x] `DECLARE`（`DECLARE @v type [(size)] [= value] [, ...]`；`@t TABLE (col defs [, table constraints])` 表變數（`IsTable`/`TableColumns`/`TableConstraints`，新 `Parse_ParenthesizedTableElements` 同時收欄位與 table 約束 PK/UNIQUE/CHECK/FK，`IsTableConstraintStart` 分流）；`{@c|name} CURSOR [opt...] [FOR <select>]` 游標（`IsCursor`/`CursorSource`/`CursorOptions`，`Parse_CursorDeclaration`，選項 LOCAL/GLOBAL/SCROLL/STATIC/KEYSET/DYNAMIC/FAST_FORWARD/READ_ONLY… 走 `TryMatchCursorOption`）；`SqlDeclareStatement` + `SqlVariableDeclaration`；ISO 式前置 `DECLARE c [INSENSITIVE] [SCROLL] CURSOR FOR ...`（`ReadIsoCursorPrefix` 在讀 dataType 前收 INSENSITIVE/SCROLL，遇 CURSOR 則走 `Parse_CursorDeclaration(leadingOptions)`，否則 reset）。）
- [x] 游標操作 `OPEN` / `CLOSE` / `DEALLOCATE`（單一 `SqlCursorOperationStatement` + `SqlCursorOperation` enum）、`FETCH [NEXT|PRIOR|FIRST|LAST|ABSOLUTE n|RELATIVE n] [FROM] cur [INTO @v[, ...]]`（`SqlFetchStatement`）；`@@FETCH_STATUS` 等全域變數已可用於 WHILE 條件（見 §5）；`GLOBAL` 游標（`OPEN/CLOSE/DEALLOCATE GLOBAL c`、`FETCH … FROM GLOBAL c`，`IsGlobal` 旗標，TryKeyword("GLOBAL") 後再讀游標名）
...
- [x] `BEGIN ... END`（`SqlBlockStatement`，以共用 `ParseStatementsUntil` 解析 body；`BEGIN TRY`/`BEGIN TRAN` 由各自 parser 在前處理）
...
- [x] `BEGIN / COMMIT / ROLLBACK / SAVE TRANSACTION`（`SqlTransactionStatement` + `SqlTransactionAction` 單類別 enum；`BEGIN|SAVE TRAN[SACTION]`、`COMMIT|ROLLBACK [TRAN|TRANSACTION|WORK]`、`BEGIN [DISTRIBUTED] TRAN[SACTION]`（`IsDistributed` 旗標）、選擇性交易名稱（含 `@var`，stop-set 擋後續語句關鍵字）、`WITH MARK ['desc']`（`WithMark`/`MarkDescription`）、`WITH (DELAYED_DURABILITY = ON)`（`WithOptions`，WITH 後 peek `(` 走 `ReadParenthesizedAssignmentList`））
- [x] `EXEC`（一般預存程序）：`{EXEC|EXECUTE} proc [arg, ...]`；具名參數 `@p = val [OUTPUT]`（`SqlExecArgument`，positional 仍為裸運算式）；動態 SQL `EXEC ('sql' | @sql)`（`SqlExecStatement.DynamicSql`）；回傳值擷取 `EXEC @ret = proc args`（`ReturnVariable`，偵測 `@var =` 前綴，否則 reset 落回變數 EXEC）；`EXEC (...) AT linked_server`（`AtLinkedServer`）。`Parse_ExecStatement` → `SqlExecStatement`
- [x] `USE <db>`（`SqlUseStatement`；`USE database_name`）
- [x] `GO`（批次分隔）（`SqlGoStatement`；選擇性 `GO count`）
- [x] `GRANT / REVOKE / DENY`（單一 `SqlPermissionStatement` + `SqlPermissionAction` enum；`{GRANT|REVOKE|DENY} perm[, ...] [ON securable] {TO|FROM} principal[, ...] [WITH GRANT OPTION] [CASCADE]`。含 `ON class::securable` 前綴（`SecurableClass`，如 `OBJECT::dbo.Orders`）、多字權限（`VIEW DEFINITION`/`CREATE TABLE`，`ReadPermissionNames` 逐字收集至 `,`/`ON`/`TO`/`FROM`）、`REVOKE GRANT OPTION FOR ...`（`GrantOptionFor`）、結尾 `AS grantor`（`AsGrantor`）、欄位層級 `GRANT SELECT (col1, col2) ON ...`（`Columns`，permission 後 peek `(`））
...
- [x] `RAISERROR`（`SqlRaiseErrorStatement`；`RAISERROR (msg, severity, state [, args...]) [WITH opt[, ...]]`，多餘參數入 `Arguments`、WITH 選項入 `Options`；格式符 `%d`/`%s` 屬訊息字面值,已隨字串保留可 round-trip。格式符與參數數量的語意檢查為 SQL Server **runtime** 行為（parse/compile 期不檢查、且寬鬆），非 T-SQL parser 職責 → N/A，不在 parser 實作）
...
- [x] `ALTER COLUMN` 的 `ADD/DROP` 子選項（`ALTER TABLE t ALTER COLUMN c {ADD|DROP} {ROWGUIDCOL|PERSISTED|SPARSE|NOT FOR REPLICATION|...}`；`SqlAlterTableColumnOption`（`ColumnName`/`IsAdd`/`Option`），`TryParseAlterColumnOption` peek 欄位名後遇 ADD/DROP 即分流、選項走 `ReadActionTokens`，否則 reset 落回欄位重定義）
...
- [x] `OPTION (query hint)`（支援 bare hint、`MAXDOP n` 數值、括號參數 hint、多 hint；hint 名稱依 `QueryHints` 白名單驗證合法性，未知 hint 回 parse error（對齊 SQL Server compile 期行為）；白名單涵蓋 `HASH/ORDER GROUP`、`CONCAT/HASH/MERGE UNION`、`LOOP/MERGE/HASH JOIN`、`FORCE ORDER`、`MAXDOP`/`MAXRECURSION`/`FAST`、`OPTIMIZE FOR [UNKNOWN]`、`PARAMETERIZATION SIMPLE/FORCED`、`USE HINT/PLAN`、`RECOMPILE`、`KEEP[FIXED] PLAN`、`ROBUST PLAN`、`QUERYTRACEON`、`TABLE HINT` 等，新增 hint 直接擴充清單）
...
- [x] 具名 `WINDOW` 子句（SQL Server 2022+；`SqlWindowClause`/`SqlWindowDefinition` 掛在 `SelectStatement.Window`，於 HAVING 後、ORDER BY 前）
...
- [x] 安全性物件：`CREATE {LOGIN | USER | ROLE}`（單一 `SqlCreatePrincipalStatement` + `SqlPrincipalKind`；ROLE `AUTHORIZATION`、USER `FOR LOGIN`、LOGIN `WITH PASSWORD =`/`FROM WINDOWS`）；`ALTER ROLE name {ADD|DROP} MEMBER member | WITH NAME = newName}`（`SqlAlterRoleStatement`，`NewName` 改名）；`ALTER {LOGIN|USER} name {ENABLE|DISABLE | WITH opt = val[, ...]}`（`SqlAlterPrincipalStatement`，`ReadAssignmentOptionList` 讀非括號 `K = V` 清單）；`DROP {LOGIN|USER|ROLE}`（見 §1 DROP））
- [x] `CREATE STATISTICS name ON t (cols)` / `UPDATE STATISTICS t [name]`（單一 `SqlStatisticsStatement`，`IsCreate` 旗標；`WITH FULLSCAN`/`NORECOMPUTE`/`SAMPLE n PERCENT` 等選項走 `ReadStatisticsWithOptions`（逗號分隔、每項收 token）存入 `Options`）
