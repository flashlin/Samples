...
> 最後驗證：2026-06-21（依 `T1.SqlSharp/ParserLit/SqlParser.cs`、`LinqParser.cs` 與測試實際比對；362 測試全綠）。
...
- [~] `CREATE VIEW` / `ALTER VIEW`（`{CREATE [OR ALTER]|ALTER} VIEW v [(cols)] [WITH opt[, ...]] AS <query> [WITH CHECK OPTION]`；body 走 `Parse_CteBodyStatement` 故支援 CTE/SELECT；AS 前 `WITH opt`（SCHEMABINDING/ENCRYPTION…）入 `Options`（共用 `Parse_WithOptionList`）。`SqlCreateViewStatement`（`IsAlter` 旗標）。未做多字選項）
...
- [~] `CREATE PROCEDURE` / `ALTER PROCEDURE`（`{CREATE [OR ALTER]|ALTER} {PROCEDURE|PROC} name [([@p type [(size)] [= default] [OUTPUT]] , ...)] AS <body>`；AS 前 `WITH opt[, ...]`（ENCRYPTION/RECOMPILE…）入 `Options`；body 走 `Parse()`（單一語句／`BEGIN…END`）。`SqlCreateProcedureStatement`（`IsAlter` 旗標）+ `SqlProcedureParameter`。未做 `FOR REPLICATION`、`EXECUTE AS`、無 BEGIN 的多裸語句 body）
- [~] `CREATE FUNCTION` / `ALTER FUNCTION`（scalar：`RETURNS type[(size)] AS <body>`；inline TVF：`RETURNS TABLE AS RETURN (select)`；multi-statement TVF：`RETURNS @t TABLE (col defs) AS BEGIN…RETURN END`（`ReturnTableVariable`/`ReturnTableColumns`，欄位重用 `ParseColumnDefinition`）。AS 前 `WITH opt[, ...]`（SCHEMABINDING…）入 `Options`。`SqlCreateFunctionStatement`（`IsAlter` 旗標），body 走 `Parse()`、return clause 抽 `ParseFunctionReturnClause`。未做多字選項 `RETURNS NULL ON NULL INPUT`）
...
2. 🟢 DDL（CREATE+ALTER VIEW/PROC/FUNCTION（含 multi-statement TVF）/TRIGGER（含 `WITH` 選項）、SCHEMA/DATABASE、ALTER TABLE 第二階段（含 ADD 混合）已完成）：多字 WITH 選項（`EXECUTE AS`/`RETURNS NULL ON NULL INPUT`）、DATABASE/SCHEMA 進階選項
