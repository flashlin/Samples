...
> 入口：`SqlParser.Parse()` dispatch ~19 種頂層語句（WITH CTE / CREATE TABLE|VIEW|INDEX / SELECT / INSERT / UPDATE / DELETE / MERGE / TRUNCATE / DROP / ALTER TABLE / EXEC sp_addextendedproperty / EXEC proc / DECLARE / BEGIN…END / IF / WHILE / SET）。
...
- [ ] `CREATE PROCEDURE` / `FUNCTION` / `TRIGGER` / `SCHEMA` / `DATABASE`
...
2. 🟢 DDL：`CREATE PROCEDURE|FUNCTION|TRIGGER`（body 重用控制流程 + DML statement parser）、ALTER TABLE 第二階段（見 §1.5）
