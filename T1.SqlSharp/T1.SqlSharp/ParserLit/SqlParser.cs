...
public class SqlParser
{
    ...
    public ParseResult<ISqlExpression> Parse()
    {
        ...
        if (Try(ParseXmlNamespacesStatement, out var xmlNamespacesStatement))
        {
            return xmlNamespacesStatement.Result;
        }
        ...
        if (Try(ParseCreateSequenceStatement, out var createSequenceStatement))
        {
            return createSequenceStatement.Result;
        }

        if (Try(ParseCreateTypeStatement, out var createTypeStatement))
        {
            return createTypeStatement.Result;
        }

        if (Try(ParseCreateSynonymStatement, out var createSynonymStatement))
        {
            return createSynonymStatement.Result;
        }

        if (Try(ParseCreatePrincipalStatement, out var createPrincipalStatement))
        {
            return createPrincipalStatement.Result;
        }

        if (Try(ParseCreateFulltextIndexStatement, out var createFulltextIndexStatement))
        {
            return createFulltextIndexStatement.Result;
        }

        if (Try(ParseCreatePartitionFunctionStatement, out var createPartitionFunctionStatement))
        {
            return createPartitionFunctionStatement.Result;
        }

        if (Try(ParseCreatePartitionSchemeStatement, out var createPartitionSchemeStatement))
        {
            return createPartitionSchemeStatement.Result;
        }

        if (Try(ParseCreateXmlSchemaCollectionStatement, out var createXmlSchemaCollectionStatement))
        {
            return createXmlSchemaCollectionStatement.Result;
        }

        if (Try(ParseCreateCertificateStatement, out var createCertificateStatement))
        {
            return createCertificateStatement.Result;
        }

        if (Try(ParseCreateMasterKeyStatement, out var createMasterKeyStatement))
        {
            return createMasterKeyStatement.Result;
        }

        if (Try(ParseSetUserStatement, out var setUserStatement))
        {
            return setUserStatement.Result;
        }

        if (Try(ParseCreateFulltextCatalogStatement, out var createFulltextCatalogStatement))
        {
            return createFulltextCatalogStatement.Result;
        }

        if (Try(ParseCreateFulltextStoplistStatement, out var createFulltextStoplistStatement))
        {
            return createFulltextStoplistStatement.Result;
        }
        ...
        if (Try(ParseStatisticsStatement, out var statisticsStatement))
        {
            return statisticsStatement.Result;
        }
        ...
        if (Try(ParseAlterIndexStatement, out var alterIndexStatement))
        {
            return alterIndexStatement.Result;
        }

        if (Try(ParseAlterSequenceStatement, out var alterSequenceStatement))
        {
            return alterSequenceStatement.Result;
        }

        if (Try(ParseAlterFulltextIndexStatement, out var alterFulltextIndexStatement))
        {
            return alterFulltextIndexStatement.Result;
        }

        if (Try(ParseAlterServerConfigurationStatement, out var alterServerConfigurationStatement))
        {
            return alterServerConfigurationStatement.Result;
        }

        if (Try(ParseAlterDatabaseStatement, out var alterDatabaseStatement))
        {
            return alterDatabaseStatement.Result;
        }

        if (Try(ParseAlterSchemaStatement, out var alterSchemaStatement))
        {
            return alterSchemaStatement.Result;
        }

        if (Try(ParseAlterAuthorizationStatement, out var alterAuthorizationStatement))
        {
            return alterAuthorizationStatement.Result;
        }

        if (Try(ParseAlterRoleStatement, out var alterRoleStatement))
        {
            return alterRoleStatement.Result;
        }

        if (Try(ParseAlterLoginOrUserStatement, out var alterPrincipalStatement))
        {
        ...
        if (Try(ParseSymmetricKeyStatement, out var symmetricKeyStatement))
        {
            return symmetricKeyStatement.Result;
        }
        ...
        if (Try(ParseGotoStatement, out var gotoStatement))
        {
        ...
        if (Try(ParseDbccStatement, out var dbccStatement))
        {
            return dbccStatement.Result;
        }

        if (Try(ParseBulkInsertStatement, out var bulkInsertStatement))
        {
            return bulkInsertStatement.Result;
        }

        if (Try(ParseKillStatement, out var killStatement))
        {
            return killStatement.Result;
        }

        if (Try(ParseBackupRestoreStatement, out var backupRestoreStatement))
        {
            return backupRestoreStatement.Result;
        }

        if (Try(ParseToggleTriggerStatement, out var toggleTriggerStatement))
        {
            return toggleTriggerStatement.Result;
        }
    ...
    public ParseResult<ISqlExpression> Parse_Value_As_DataType()
    {
        ...
        if (TryKeywords(["AT", "TIME", "ZONE"], out var atTimeZoneSpan))
        {
            var timeZone = ParseArithmeticExpr();
            if (timeZone.HasError)
            {
                return timeZone.Error;
            }

            value = new SqlAtTimeZoneExpr
            {
                Span = _text.CreateSpan(atTimeZoneSpan),
                Expression = value.ResultValue,
                TimeZone = timeZone.ResultValue
            };
        }
    ...
    public ParseResult<SqlCreateTableExpression> ParseCreateTableStatement()
    {
        ...
        if (!IsPeekMatch("("))
        {
            ...
            {
                var ctasOptions = Parse_ParenthesizedOptionList();
                if (ctasOptions.HasError)
                {
                    return ctasOptions.Error;
                }

                createTableStatement.WithOptions = ctasOptions.ResultValue;
            }

            if (!TryKeyword("AS", out _))
            {
                return CreateParseError("Expected ( or AS in CREATE TABLE");
            }

            var asSelect = ParseSelectStatement();
            if (asSelect.HasError)
            {
                return asSelect.Error;
            }

            createTableStatement.AsSelect = asSelect.ResultValue;
            SkipStatementEnd();
            createTableStatement.Span = _text.CreateSpan(startSpan);
            return CreateParseResult(createTableStatement);
        }

        TryMatch("(", out var openParenthesis);
        ...
        while (!_text.IsEnd())
        {
            ...
            if (TryKeywords(["PERIOD", "FOR", "SYSTEM_TIME"], out _))
            {
                if (!TryMatch("(", out _))
                {
                    return CreateParseError("Expected ( after PERIOD FOR SYSTEM_TIME");
                }
                ...
                createTableStatement.Period = string.Join(", ", periodColumns);
                if (_text.PeekChar() == ',')
                {
                    _text.ReadChar();
                }
        ...
        var tableOptions = Parse_CreateTableOptions(createTableStatement);
        if (tableOptions.HasError)
        {
    ...
    private ParseResult<bool> Parse_CreateTableOptions(SqlCreateTableExpression statement)
    {
        while (true)
        {
            if (TryKeyword("ON", out _))
            {
                statement.OnFileGroup = ReadFileGroupName();
                continue;
            }

            if (TryKeyword("TEXTIMAGE_ON", out _))
            {
                statement.TextImageOn = ReadFileGroupName();
                continue;
            }

            if (TryKeyword("FILESTREAM_ON", out _))
            {
                ReadFileGroupName();
                continue;
            }
            ...
            if (IsPeekKeywords("WITH"))
            {
                ...
                statement.WithOptions = options.ResultValue;
                continue;
            }
    ...
    private string ReadFileGroupName()
    {
        var name = Parse_SqlIdentifier();
        var fileGroup = name.Result?.FieldName ?? ReadSqlIdentifier().Word;
        if (TryMatch("(", out _))
        {
            var columns = ReadCommaSeparatedIdentifiers();
            TryMatch(")", out _);
            fileGroup += $" ({string.Join(", ", columns)})";
        }
    ...
    private ParseResult<SqlXmlNamespacesStatement> ParseXmlNamespacesStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("WITH", out var startSpan))
        {
            return NoneResult<SqlXmlNamespacesStatement>();
        }

        if (!TryKeyword("XMLNAMESPACES", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlXmlNamespacesStatement>();
        }
        ...
        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( after XMLNAMESPACES");
        }
        ...
        do
        {
            ...
            {
                var uri = Try(ParseSqlQuotedString, out var defaultUri) ? defaultUri.ResultValue.ToSql() : string.Empty;
                namespaces.Add($"DEFAULT {uri}");
            }
            else
            {
                var uri = Try(ParseSqlQuotedString, out var nsUri) ? nsUri.ResultValue.ToSql() : string.Empty;
                TryKeyword("AS", out _);
                var prefix = ReadSqlIdentifier().Word;
                namespaces.Add($"{uri} AS {prefix}");
            }
        } while (TryMatch(",", out _));

        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) to close XMLNAMESPACES");
        }

        var body = Parse_CteBodyStatement();
        if (body.HasError)
        {
            return body.Error;
        }

        return CreateParseResult(new SqlXmlNamespacesStatement
        {
            Span = _text.CreateSpan(startSpan),
    ...
    private ParseResult<List<string>> ReadOpenJsonSchema()
    {
        if (!TryKeyword("WITH", out _) || !TryMatch("(", out _))
        {
            return CreateParseError("Expected WITH ( for OPENJSON schema");
        }

        var columns = new List<string>();
        do
        {
            var parts = new List<string> { ReadSqlIdentifier().Word };
            var type = ReadSqlIdentifier().Word;
            var size = Parse_DataSize();
            if (size.Result != null)
            {
                type += $"({size.ResultValue.Size})";
            }

            parts.Add(type);

            if (Try(ParseSqlQuotedString, out var path))
            {
                parts.Add(path.ResultValue.ToSql());
            }

            if (TryKeywords(["AS", "JSON"], out _))
            {
                parts.Add("AS JSON");
            }
        ...
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) to close OPENJSON schema");
        }
    ...
    private ParseResult<ITableSource> Parse_FromTableSource()
    {
        ...
        if (Try(ParseFunctionCall, out var function))
        {
            var funcSource = new SqlFuncTableSource()
            {
                Span = function.ResultValue.Span,
                Function = function.ResultValue
            };

            if (string.Equals(function.ResultValue.FunctionName, "OPENJSON", StringComparison.OrdinalIgnoreCase)
                && IsPeekKeywords("WITH"))
            {
                var schema = ReadOpenJsonSchema();
                if (schema.HasError)
                {
                    return schema.Error;
                }

                funcSource.JsonSchemaColumns = schema.ResultValue;
            }
    ...
    private ParseResult<ISqlExpression> Parse_FunctionArgument()
    {
        if (TryKeyword("BULK", out _))
        {
        ...
        if (TryMatch(":", out _))
        {
            ...
            return CreateParseResult<ISqlExpression>(new SqlAssignExpr
            {
                Left = expr.ResultValue,
                Operator = ":",
                Right = value.ResultValue
            });
        }
    ...
    private ParseResult<SqlFunctionExpression> ParseFunctionCall()
    {
        ...
        if (TryReadSqlFunctionName(out var identifier))
        {
            if (TryMatch("(", out var openParenthesis))
            {
                var parameters = ParseWithComma(Parse_FunctionArgument);
                if (!TryMatch(")", out _))
                {
                    return CreateParseError("Expected )");
                }
    ...
    private ParseResult<SqlExecStatement> Parse_ExecStatement()
    {
        ...
        var returnVariable = string.Empty;
        var beforeReturnVariable = _text.Position;
        var firstIdentifier = Parse_SqlIdentifier();
        if (firstIdentifier.Result != null
            && firstIdentifier.ResultValue.FieldName.StartsWith("@")
            && TryMatch("=", out _))
        {
            returnVariable = firstIdentifier.ResultValue.FieldName;
        }
        else
        {
            _text.Position = beforeReturnVariable;
        }
    ...
    private ParseResult<SqlExecStatement> Parse_ExecDynamicSql(TextSpan startSpan)
    {
        ...
        var atLinkedServer = string.Empty;
        if (TryKeyword("AT", out _))
        {
            atLinkedServer = Parse_SqlIdentifier().Result?.FieldName ?? string.Empty;
        }
    ...
    private ParseResult<SqlAlterSchemaStatement> ParseAlterSchemaStatement()
    {
        if (!TryKeywords(["ALTER", "SCHEMA"], out var startSpan))
        {
            return NoneResult<SqlAlterSchemaStatement>();
        }

        var schemaName = Parse_SqlIdentifier();
        if (schemaName.Result == null)
        {
            return CreateParseError("Expected schema name after ALTER SCHEMA");
        }

        if (!TryKeyword("TRANSFER", out _))
        {
            return CreateParseError("Expected TRANSFER in ALTER SCHEMA");
        }

        TryKeywords(["OBJECT", "::"], out _);

        var objectName = Parse_SqlIdentifier();
        if (objectName.Result == null)
        {
            return CreateParseError("Expected object name after TRANSFER");
        }

        return CreateParseResult(new SqlAlterSchemaStatement
        {
            Span = _text.CreateSpan(startSpan),
            SchemaName = schemaName.ResultValue.FieldName,
            ObjectName = objectName.ResultValue.FieldName
        });
    }

    private ParseResult<SqlAlterDatabaseStatement> ParseAlterDatabaseStatement()
    {
        if (!TryKeywords(["ALTER", "DATABASE"], out var startSpan))
        {
            return NoneResult<SqlAlterDatabaseStatement>();
        }

        var databaseName = TryKeyword("CURRENT", out _) ? "CURRENT" : Parse_SqlIdentifier().ResultValue?.FieldName;
    ...
    private ParseResult<SqlAlterIndexStatement> ParseAlterIndexStatement()
    {
        if (!TryKeywords(["ALTER", "INDEX"], out var startSpan))
        {
            return NoneResult<SqlAlterIndexStatement>();
        }

        var indexName = TryKeyword("ALL", out _) ? "ALL" : Parse_SqlIdentifier().ResultValue?.FieldName;
        if (string.IsNullOrEmpty(indexName))
        {
            return CreateParseError("Expected index name after ALTER INDEX");
        }

        if (!TryKeyword("ON", out _))
        {
            return CreateParseError("Expected ON in ALTER INDEX");
        }

        var tableName = Parse_SqlIdentifier();
        if (tableName.Result == null)
        {
            return CreateParseError("Expected table name after ON in ALTER INDEX");
        }
        ...
        else if (TryKeyword("SET", out _))
        {
            action = "SET";
            var optionList = ReadParenthesizedAssignmentList();
            if (optionList.HasError)
            {
                return optionList.Error;
            }

            options = optionList.ResultValue;
        }
        else
        {
            return CreateParseError("Expected REBUILD, REORGANIZE, DISABLE or SET in ALTER INDEX");
        }

        return CreateParseResult(new SqlAlterIndexStatement
        {
            Span = _text.CreateSpan(startSpan),
            IndexName = indexName,
            TableName = tableName.ResultValue.FieldName,
    ...
    {
        if (!TryKeywords(["ALTER", "AUTHORIZATION", "ON"], out var startSpan))
        {
            return NoneResult<SqlAlterAuthorizationStatement>();
        }

        var securable = Parse_SqlIdentifier();
        if (securable.Result == null)
        {
            return CreateParseError("Expected securable after ALTER AUTHORIZATION ON");
        }

        var securableClass = string.Empty;
        var objectName = securable.ResultValue.FieldName;
        if (TryMatch("::", out _))
        {
            securableClass = objectName;
            var target = Parse_SqlIdentifier();
            if (target.Result == null)
            {
                return CreateParseError("Expected securable name after ::");
            }

            objectName = target.ResultValue.FieldName;
        }

        if (!TryKeyword("TO", out _))
        {
            return CreateParseError("Expected TO in ALTER AUTHORIZATION");
        }

        var principal = ReadSqlIdentifier().Word;
        if (string.IsNullOrEmpty(principal))
        {
            return CreateParseError("Expected principal in ALTER AUTHORIZATION");
        }

        return CreateParseResult(new SqlAlterAuthorizationStatement
        {
            Span = _text.CreateSpan(startSpan),
    ...
    private ParseResult<SqlAlterRoleStatement> ParseAlterRoleStatement()
    {
        if (!TryKeywords(["ALTER", "ROLE"], out var startSpan))
        {
            return NoneResult<SqlAlterRoleStatement>();
        }

        var roleName = Parse_SqlIdentifier();
        if (roleName.Result == null)
        {
        ...
        else if (TryKeywords(["DROP", "MEMBER"], out _))
        {
            isAddMember = false;
        }
        else
        ...
        if (memberName.Result == null)
        {
            return CreateParseError("Expected member name in ALTER ROLE");
        }
        ...
        return CreateParseResult(new SqlAlterRoleStatement
        {
            ...
            MemberName = memberName.ResultValue.FieldName
        });
    }

    private ParseResult<SqlAlterPrincipalStatement> ParseAlterLoginOrUserStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("ALTER", out var startSpan))
        {
            return NoneResult<SqlAlterPrincipalStatement>();
        }

        SqlPrincipalKind kind;
        if (TryKeyword("LOGIN", out _))
        {
            kind = SqlPrincipalKind.Login;
        }
        else if (TryKeyword("USER", out _))
        {
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected principal name after ALTER");
        }

        var statement = new SqlAlterPrincipalStatement
        {
            Span = _text.CreateSpan(startSpan),
            Kind = kind,
            Name = name.ResultValue.FieldName
        ...
        {
            statement.Action = "ENABLE";
        }
        else if (TryKeyword("DISABLE", out _))
        {
            statement.Action = "DISABLE";
        }
        else if (TryKeyword("WITH", out _))
        {
    ...
    private List<string> ReadAssignmentOptionList()
    {
        var options = new List<string>();
        do
        {
            ...
            {
                var value = ParseArithmeticExpr();
                options.Add($"{key} = {value.ResultValue?.ToSql()}");
            }
            else
            {
                options.Add(key);
    ...
    private static readonly Dictionary<string, SqlPrincipalKind> PrincipalKeywords = new(StringComparer.OrdinalIgnoreCase)
    {
        ["LOGIN"] = SqlPrincipalKind.Login,
        ["USER"] = SqlPrincipalKind.User,
        ["ROLE"] = SqlPrincipalKind.Role
    };

    private ParseResult<SqlCreatePrincipalStatement> ParseCreatePrincipalStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreatePrincipalStatement>();
        }

        SqlPrincipalKind kind = default;
        var matched = false;
        foreach (var entry in PrincipalKeywords)
        {
            if (TryKeyword(entry.Key, out _))
            {
                kind = entry.Value;
                matched = true;
                break;
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected principal name after CREATE");
        }

        var statement = new SqlCreatePrincipalStatement
        {
            Span = _text.CreateSpan(startSpan),
            Kind = kind,
            Name = name.ResultValue.FieldName
        ...
        {
            statement.Authorization = ReadSqlIdentifier().Word;
        }
        else if (TryKeywords(["FOR", "LOGIN"], out _))
        {
            statement.ForLogin = ReadSqlIdentifier().Word;
        }
        else if (TryKeywords(["WITH", "PASSWORD"], out _) && TryMatch("=", out _))
        {
            var password = ParseArithmeticExpr();
    ...
    private ParseResult<SqlStatisticsStatement> ParseStatisticsStatement()
    {
        var startPosition = _text.Position;
        bool isCreate;
        if (TryKeyword("CREATE", out var startSpan))
        ...
        else if (TryKeyword("UPDATE", out startSpan))
        {
            isCreate = false;
        }
        else
        {
            return NoneResult<SqlStatisticsStatement>();
        }

        if (!TryKeyword("STATISTICS", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlStatisticsStatement>();
        }
        ...
        if (isCreate)
        {
            ...
            if (name.Result == null)
            {
                return CreateParseError("Expected statistics name after CREATE STATISTICS");
            }

            if (!TryKeyword("ON", out _))
            {
                return CreateParseError("Expected ON in CREATE STATISTICS");
            }

            var tableName = Parse_SqlIdentifier();
            if (tableName.Result == null)
            {
                return CreateParseError("Expected table name after ON");
            }
            ...
            return CreateParseResult(new SqlStatisticsStatement
            {
                Span = _text.CreateSpan(startSpan),
                IsCreate = true,
                Name = name.ResultValue.FieldName,
        ...
        var updateTable = Parse_SqlIdentifier();
        if (updateTable.Result == null)
        {
            return CreateParseError("Expected table name after UPDATE STATISTICS");
        }

        var statistics = new SqlStatisticsStatement
        {
            Span = _text.CreateSpan(startSpan),
            IsCreate = false,
    ...
    private ParseResult<SqlCreateSynonymStatement> ParseCreateSynonymStatement()
    {
        ...
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateSynonymStatement>();
        }

        if (!TryKeyword("SYNONYM", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateSynonymStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected synonym name after CREATE SYNONYM");
        }

        if (!TryKeyword("FOR", out _))
        {
            return CreateParseError("Expected FOR in CREATE SYNONYM");
        }

        var forName = Parse_SqlIdentifier();
        if (forName.Result == null)
        {
            return CreateParseError("Expected target name after FOR");
        }

        return CreateParseResult(new SqlCreateSynonymStatement
        {
            Span = _text.CreateSpan(startSpan),
            SynonymName = name.ResultValue.FieldName,
            ForName = forName.ResultValue.FieldName
        });
    }

    private ParseResult<SqlCreateTypeStatement> ParseCreateTypeStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateTypeStatement>();
        }

        if (!TryKeyword("TYPE", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateTypeStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected type name after CREATE TYPE");
        }

        var statement = new SqlCreateTypeStatement
        {
            Span = _text.CreateSpan(startSpan),
            TypeName = name.ResultValue.FieldName
        };

        if (TryKeywords(["AS", "TABLE"], out _))
        {
            ...
            statement.IsTable = true;
            statement.TableColumns = columns.ResultValue;
            return CreateParseResult(statement);
        }

        if (!TryKeyword("FROM", out _))
        {
            return CreateParseError("Expected AS TABLE or FROM in CREATE TYPE");
        }

        statement.BaseType = ReadSqlIdentifier().Word;
        var baseSize = Parse_DataSize();
        if (baseSize.HasError)
        {
            return baseSize.Error;
        }

        statement.BaseSize = baseSize.Result;
        return CreateParseResult(statement);
    }

    private ParseResult<SqlCreateFulltextIndexStatement> ParseCreateFulltextIndexStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateFulltextIndexStatement>();
        }

        if (!TryKeywords(["FULLTEXT", "INDEX"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateFulltextIndexStatement>();
        }
        ...
        if (!TryKeyword("ON", out _))
        {
            return CreateParseError("Expected ON in CREATE FULLTEXT INDEX");
        }

        var tableName = Parse_SqlIdentifier();
        if (tableName.Result == null)
        {
            return CreateParseError("Expected table name in CREATE FULLTEXT INDEX");
        }

        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for fulltext columns");
        }

        var columns = ReadCommaSeparatedIdentifiers();
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) after fulltext columns");
        }

        var statement = new SqlCreateFulltextIndexStatement
        {
            Span = _text.CreateSpan(startSpan),
            TableName = tableName.ResultValue.FieldName,
            Columns = columns
        ...
        if (TryKeywords(["KEY", "INDEX"], out _))
        {
            var keyIndex = Parse_SqlIdentifier();
            if (keyIndex.Result == null)
            {
                return CreateParseError("Expected key index name");
            }

            statement.KeyIndex = keyIndex.ResultValue.FieldName;
        }

        if (TryKeyword("ON", out _))
        {
            var catalog = Parse_SqlIdentifier();
            if (catalog.Result == null)
            {
                return CreateParseError("Expected catalog name after ON");
            }

            statement.Catalog = catalog.ResultValue.FieldName;
        }

        return CreateParseResult(statement);
    }

    private ParseResult<SqlCreatePartitionFunctionStatement> ParseCreatePartitionFunctionStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreatePartitionFunctionStatement>();
        }

        if (!TryKeywords(["PARTITION", "FUNCTION"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreatePartitionFunctionStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected partition function name");
        }

        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for input type");
        }

        var inputType = ReadSqlIdentifier().Word;
        var size = Parse_DataSize();
        if (size.Result != null)
        {
            inputType += $"({size.ResultValue.Size})";
        }

        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) after input type");
        }

        if (!TryKeywords(["AS", "RANGE"], out _))
        {
            return CreateParseError("Expected AS RANGE in CREATE PARTITION FUNCTION");
        }

        var rangeDirection = string.Empty;
        if (TryKeyword("LEFT", out _))
        {
            rangeDirection = "LEFT";
        }
        else if (TryKeyword("RIGHT", out _))
        {
            rangeDirection = "RIGHT";
        }

        if (!TryKeywords(["FOR", "VALUES"], out _))
        {
            return CreateParseError("Expected FOR VALUES in CREATE PARTITION FUNCTION");
        }

        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for boundary values");
        }

        var values = ParseWithComma(ParseArithmetic_Primary);
        if (values.HasError)
        {
            return values.Error;
        }
        ...
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) after boundary values");
        }

        return CreateParseResult(new SqlCreatePartitionFunctionStatement
        {
            Span = _text.CreateSpan(startSpan),
            FunctionName = name.ResultValue.FieldName,
            InputType = inputType,
            RangeDirection = rangeDirection,
            BoundaryValues = values.ResultValue.Select(value => value.ToSql()).ToList()
        });
    }

    private ParseResult<SqlCreatePartitionSchemeStatement> ParseCreatePartitionSchemeStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreatePartitionSchemeStatement>();
        }

        if (!TryKeywords(["PARTITION", "SCHEME"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreatePartitionSchemeStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected partition scheme name");
        }

        if (!TryKeywords(["AS", "PARTITION"], out _))
        {
            return CreateParseError("Expected AS PARTITION in CREATE PARTITION SCHEME");
        }

        var partitionFunction = Parse_SqlIdentifier();
        if (partitionFunction.Result == null)
        {
            return CreateParseError("Expected partition function name");
        }

        var allToOne = TryKeyword("ALL", out _);

        if (!TryKeyword("TO", out _))
        {
            return CreateParseError("Expected TO in CREATE PARTITION SCHEME");
        }

        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for filegroups");
        }

        var fileGroups = ReadCommaSeparatedIdentifiers();
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) after filegroups");
        }

        return CreateParseResult(new SqlCreatePartitionSchemeStatement
        {
            Span = _text.CreateSpan(startSpan),
            SchemeName = name.ResultValue.FieldName,
            PartitionFunction = partitionFunction.ResultValue.FieldName,
            AllToOneFileGroup = allToOne,
            FileGroups = fileGroups
        });
    }

    private ParseResult<SqlCreateXmlSchemaCollectionStatement> ParseCreateXmlSchemaCollectionStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateXmlSchemaCollectionStatement>();
        }

        if (!TryKeywords(["XML", "SCHEMA", "COLLECTION"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateXmlSchemaCollectionStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected XML schema collection name");
        }

        if (!TryKeyword("AS", out _))
        {
            return CreateParseError("Expected AS in CREATE XML SCHEMA COLLECTION");
        }

        var schema = ParseArithmetic_Primary();
        if (schema.Result == null)
        {
            return CreateParseError("Expected schema expression in CREATE XML SCHEMA COLLECTION");
        }

        return CreateParseResult(new SqlCreateXmlSchemaCollectionStatement
        {
            Span = _text.CreateSpan(startSpan),
            Name = name.ResultValue.FieldName,
    ...
    private ParseResult<SqlCreateFulltextCatalogStatement> ParseCreateFulltextCatalogStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateFulltextCatalogStatement>();
        }

        if (!TryKeywords(["FULLTEXT", "CATALOG"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateFulltextCatalogStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected fulltext catalog name");
        }

        var statement = new SqlCreateFulltextCatalogStatement
        {
            Span = _text.CreateSpan(startSpan),
            Name = name.ResultValue.FieldName
        };

        if (TryKeywords(["AS", "DEFAULT"], out _))
        {
    ...
    private ParseResult<SqlCreateFulltextStoplistStatement> ParseCreateFulltextStoplistStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateFulltextStoplistStatement>();
        }

        if (!TryKeywords(["FULLTEXT", "STOPLIST"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateFulltextStoplistStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected fulltext stoplist name");
        }

        var statement = new SqlCreateFulltextStoplistStatement
        {
            Span = _text.CreateSpan(startSpan),
            Name = name.ResultValue.FieldName
        };
        ...
        if (TryKeyword("FROM", out _))
        {
            if (TryKeywords(["SYSTEM", "STOPLIST"], out _))
            {
                statement.Source = "SYSTEM STOPLIST";
            }
            else
            {
                statement.Source = Parse_SqlIdentifier().Result?.FieldName ?? string.Empty;
            }
        }
    ...
    private ParseResult<SqlCreateSymmetricKeyStatement> ParseCreateSymmetricKeyStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateSymmetricKeyStatement>();
        }

        if (!TryKeywords(["SYMMETRIC", "KEY"], out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateSymmetricKeyStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected symmetric key name");
        }

        var options = new List<string>();
        if (TryKeyword("WITH", out _))
        {
            options.AddRange(ReadAssignmentOptionList());
        }

        if (TryKeywords(["ENCRYPTION", "BY"], out _))
        {
            var clause = "ENCRYPTION BY " + ReadSqlIdentifier().Word;
            if (TryMatch("=", out _))
            {
                clause += " = " + (ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty);
            }
            else
            {
                var encryptor = Parse_SqlIdentifier();
                if (encryptor.Result != null)
                {
                    clause += " " + encryptor.ResultValue.FieldName;
                }
            }
        ...
        return CreateParseResult(new SqlCreateSymmetricKeyStatement
        {
            Span = _text.CreateSpan(startSpan),
            Name = name.ResultValue.FieldName,
            Options = options
    ...
    private ParseResult<SqlSymmetricKeyStatement> ParseSymmetricKeyStatement()
    {
        var startPosition = _text.Position;
        bool isOpen;
        TextSpan startSpan;
        ...
        {
            return NoneResult<SqlSymmetricKeyStatement>();
        }

        if (TryKeywords(["ALL", "SYMMETRIC", "KEYS"], out _))
        {
            return CreateParseResult(new SqlSymmetricKeyStatement
            {
                Span = _text.CreateSpan(startSpan),
                IsOpen = isOpen,
                AllKeys = true
        ...
        var keyName = Parse_SqlIdentifier();
        if (keyName.Result == null)
        {
            return CreateParseError("Expected symmetric key name");
        }

        var statement = new SqlSymmetricKeyStatement
        {
            Span = _text.CreateSpan(startSpan),
            IsOpen = isOpen,
            KeyName = keyName.ResultValue.FieldName
        };

        if (TryKeywords(["DECRYPTION", "BY"], out _))
        {
            var clause = ReadSqlIdentifier().Word;
            if (TryMatch("=", out _))
            {
                clause += " = " + (ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty);
            }
            ...
            {
                var by = Parse_SqlIdentifier();
                if (by.Result != null)
                {
                    clause += " " + by.ResultValue.FieldName;
                }
            }

            statement.DecryptionBy = clause;
        }
    ...
    {
        if (!TryKeywords(["ALTER", "FULLTEXT", "INDEX"], out var startSpan))
        {
            return NoneResult<SqlAlterFulltextIndexStatement>();
        }

        if (!TryKeyword("ON", out _))
        {
            return CreateParseError("Expected ON in ALTER FULLTEXT INDEX");
        }

        var tableName = Parse_SqlIdentifier();
        if (tableName.Result == null)
        {
            return CreateParseError("Expected table name in ALTER FULLTEXT INDEX");
        }
        ...
        return CreateParseResult(new SqlAlterFulltextIndexStatement
        {
            Span = _text.CreateSpan(startSpan),
            TableName = tableName.ResultValue.FieldName,
            Action = string.Join(" ", actionWords)
        });
    }

    private ParseResult<SqlAlterServerConfigurationStatement> ParseAlterServerConfigurationStatement()
    {
        if (!TryKeywords(["ALTER", "SERVER", "CONFIGURATION", "SET"], out var startSpan))
        {
            return NoneResult<SqlAlterServerConfigurationStatement>();
        }

        var settingTokens = new List<string>();
        while (!_text.IsEnd() && !IsPeekMatch(";"))
        {
            if (TryMatch("=", out _))
            {
            ...
            settingTokens.Add(word);
        }

        return CreateParseResult(new SqlAlterServerConfigurationStatement
        {
            Span = _text.CreateSpan(startSpan),
            Setting = string.Join(" ", settingTokens)
        });
    }

    private ParseResult<SqlCreateCertificateStatement> ParseCreateCertificateStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateCertificateStatement>();
        }

        if (!TryKeyword("CERTIFICATE", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateCertificateStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected certificate name");
        }

        var statement = new SqlCreateCertificateStatement
        {
            Span = _text.CreateSpan(startSpan),
            Name = name.ResultValue.FieldName
        };
        ...
        while (true)
        {
            if (TryKeywords(["FROM", "FILE"], out _))
            {
                TryMatch("=", out _);
                statement.FromFile = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
                continue;
            }

            if (TryKeywords(["ENCRYPTION", "BY", "PASSWORD"], out _))
            {
                TryMatch("=", out _);
                statement.Password = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
                continue;
            }
    ...
    private ParseResult<SqlCreateMasterKeyStatement> ParseCreateMasterKeyStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateMasterKeyStatement>();
        }
        ...
        {
            TryMatch("=", out _);
            password = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
        }

        return CreateParseResult(new SqlCreateMasterKeyStatement
        {
            Span = _text.CreateSpan(startSpan),
            Password = password
        });
    ...
    private ParseResult<SqlSetUserStatement> ParseSetUserStatement()
    {
        if (!TryKeyword("SETUSER", out var startSpan))
        {
            return NoneResult<SqlSetUserStatement>();
        }

        var userName = string.Empty;
        if (!_text.IsEnd() && !IsPeekMatch(";"))
        {
            userName = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
        }

        return CreateParseResult(new SqlSetUserStatement
        {
            Span = _text.CreateSpan(startSpan),
            UserName = userName
        });
    ...
    private ParseResult<SqlAlterSequenceStatement> ParseAlterSequenceStatement()
    {
        if (!TryKeywords(["ALTER", "SEQUENCE"], out var startSpan))
        {
            return NoneResult<SqlAlterSequenceStatement>();
        }

        var name = Parse_SqlIdentifier();
        if (name.Result == null)
        {
            return CreateParseError("Expected sequence name after ALTER SEQUENCE");
        }

        var statement = new SqlAlterSequenceStatement
        {
            Span = _text.CreateSpan(startSpan),
            SequenceName = name.ResultValue.FieldName
        };

        while (true)
        {
            if (TryKeyword("RESTART", out _))
            {
                statement.IsRestart = true;
                if (TryKeyword("WITH", out _))
                {
                    var restartWith = ParseArithmeticExpr();
                    if (restartWith.HasError)
                    {
                        return restartWith.Error;
                    }

                    statement.RestartWith = restartWith.Result;
                }

                continue;
            }

            if (TryKeywords(["INCREMENT", "BY"], out _))
            {
                var incrementBy = ParseArithmeticExpr();
                if (incrementBy.HasError)
                {
                    return incrementBy.Error;
                }

                statement.IncrementBy = incrementBy.Result;
                continue;
            }
    ...
    private ParseResult<SqlCreateSequenceStatement> ParseCreateSequenceStatement()
    {
        var startPosition = _text.Position;
        if (!TryKeyword("CREATE", out var startSpan))
        {
            return NoneResult<SqlCreateSequenceStatement>();
        }

        if (!TryKeyword("SEQUENCE", out _))
        {
            _text.Position = startPosition;
            return NoneResult<SqlCreateSequenceStatement>();
        }
        ...
        if (name.Result == null)
        {
            return CreateParseError("Expected sequence name after CREATE SEQUENCE");
        }

        var statement = new SqlCreateSequenceStatement
        {
            Span = _text.CreateSpan(startSpan),
            SequenceName = name.ResultValue.FieldName
        };

        var clauses = ParseSequenceClauses(statement);
        if (clauses.HasError)
        {
    ...
    private ParseResult<bool> ParseSequenceClauses(SqlCreateSequenceStatement statement)
    {
        while (true)
        {
            if (TryKeyword("AS", out _))
            {
                statement.DataType = ReadSqlIdentifier().Word;
                continue;
            }

            if (TryKeywords(["START", "WITH"], out _))
            {
                var startWith = ParseArithmeticExpr();
                if (startWith.HasError)
                {
                    return startWith.Error;
                }

                statement.StartWith = startWith.Result;
                continue;
            }
    ...
    private ParseResult<ISqlExpression> ParseCreateSchemaOrDatabaseStatement()
    {
        ...
        if (TryKeyword("DATABASE", out _))
        {
            ...
            var databaseStatement = new SqlCreateDatabaseStatement
            {
                Span = _text.CreateSpan(startSpan),
            ...
            if (TryKeyword("ON", out _))
            {
                ...
                var dataFiles = ReadFileSpecList();
                if (dataFiles.HasError)
                {
            ...
            {
                var logFiles = ReadFileSpecList();
                if (logFiles.HasError)
                {
                    return logFiles.Error;
                }

                databaseStatement.LogFiles = logFiles.ResultValue;
            }
    ...
    private ParseResult<List<string>> ReadFileSpecList()
    {
        ...
        do
        {
            ...
            if (!TryMatch(")", out _))
            {
                return CreateParseError("Expected ) to close file specification");
            }

            specs.Add($"({string.Join(", ", parts)})");
        } while (TryMatch(",", out _));
    ...
    private ParseResult<SqlPermissionStatement> ParsePermissionStatement()
    {
        ...
        var grantOptionFor = TryKeywords(["GRANT", "OPTION", "FOR"], out _);

        var permissions = ReadPermissionNames();
        ...
        if (TryMatch("(", out _))
        {
            columns = ReadCommaSeparatedIdentifiers();
            if (!TryMatch(")", out _))
            {
                return CreateParseError("Expected ) after permission columns");
            }
        }
        ...
        if (TryKeyword("ON", out _))
        {
            ...
            if (TryMatch("::", out _))
            {
                securableClass = securable.ResultValue.FieldName;
                var target = Parse_SqlIdentifier();
                if (target.Result == null)
                {
                    return CreateParseError("Expected securable name after ::");
        ...
        var statement = new SqlPermissionStatement
        {
            Span = _text.CreateSpan(startSpan),
            Action = action,
        ...
        if (TryKeyword("AS", out _))
        {
            statement.AsGrantor = ReadSqlIdentifier().Word;
        }
    ...
    private List<string> ReadPermissionNames()
    {
        ...
        do
        {
            ...
            if (words.Count > 0)
            {
                permissions.Add(string.Join(" ", words));
            }
        } while (TryMatch(",", out _));
    ...
    private ParseResult<bool> Parse_ParenthesizedTableElements(
        out List<SqlColumnDefinition> columns, out List<ISqlConstraint> constraints)
    {
        columns = [];
        constraints = [];
        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for table definition");
        }
        ...
        do
        {
            if (IsTableConstraintStart())
            {
                ...
                {
                    return CreateParseError("Expected table constraint");
                }
            ...
            else
            {
                ...
                {
                    return CreateParseError("Expected column definition");
                }
        ...
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) after table definition");
        }
    ...
    private ParseResult<SqlCreateIndexStatement> ParseCreateIndexStatement()
    {
        ...
        var indexColumns = new List<SqlConstraintColumn>();
        if (IsPeekMatch("("))
        {
            ...
            indexColumns = columns.ResultValue;
        }
    ...
    private ParseResult<List<string>> Parse_ParenthesizedOptionList()
    {
        if (!TryKeyword("WITH", out _) || !TryMatch("(", out _))
        {
            return CreateParseError("Expected WITH ( for option list");
        }
        ...
        if (!TryMatch(")", out _))
        {
            return CreateParseError("Expected ) to close option list");
        }

        return CreateParseResult(options);
    }
    ...
    private ParseResult<ISqlAlterTableAction> Parse_AlterTableAction()
    {
        ...
        if (TryKeyword("REBUILD", out _))
        {
            var rebuild = new SqlAlterTableRebuild();
            if (TryKeyword("PARTITION", out _))
            {
                TryMatch("=", out _);
                ParseArithmetic_Primary();
            ...
            if (IsPeekKeywords("WITH"))
            {
                ...
                rebuild.Options = options.ResultValue;
            }

            return CreateParseResult<ISqlAlterTableAction>(rebuild);
        }
        ...
        if (TryKeyword("SET", out _))
        {
            ...
            return CreateParseResult<ISqlAlterTableAction>(new SqlAlterTableSet
            {
                Options = options.ResultValue
            });
        }

        if (TryKeyword("SWITCH", out _))
        {
            var sourcePartition = string.Empty;
            if (TryKeyword("PARTITION", out _))
            {
                sourcePartition = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
            }

            if (!TryKeyword("TO", out _))
            {
                return CreateParseError("Expected TO in ALTER TABLE SWITCH");
            }

            var targetTable = Parse_SqlIdentifier();
            if (targetTable.Result == null)
            {
                return CreateParseError("Expected target table in ALTER TABLE SWITCH");
            }

            var targetPartition = string.Empty;
            if (TryKeyword("PARTITION", out _))
            {
                targetPartition = ParseArithmetic_Primary().Result?.ToSql() ?? string.Empty;
            }

            return CreateParseResult<ISqlAlterTableAction>(new SqlAlterTableSwitch
            {
                SourcePartition = sourcePartition,
                TargetTable = targetTable.ResultValue.FieldName,
    ...
    private ParseResult<List<string>> ReadParenthesizedAssignmentList()
    {
        if (!TryMatch("(", out _))
        {
            return CreateParseError("Expected ( for option list");
        }
    ...
    private static readonly Dictionary<string, SqlDropObjectType> DropObjectTypes = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["SYNONYM"] = SqlDropObjectType.Synonym,
        ["LOGIN"] = SqlDropObjectType.Login,
        ["USER"] = SqlDropObjectType.User,
    ...
    private ParseResult<SqlTransactionStatement> ParseTransactionStatement()
    {
        ...
        {
            var isDistributed = TryKeyword("DISTRIBUTED", out _);
            ...
            return CreateTransactionStatement(SqlTransactionAction.Begin, beginSpan, isDistributed);
        }
    ...
    private ParseResult<SqlVariableDeclaration> Parse_VariableDeclaration()
    {
        ...
        {
            var elements = Parse_ParenthesizedTableElements(out var tableColumns, out var tableConstraints);
    ...
    private bool TryMatchCursorOption(out string option)
    {
        foreach (var keyword in CursorOptionKeywords)
        {
            if (TryKeyword(keyword, out _))
            {
                option = keyword;
    ...
    private ParseResult<SqlVariableDeclaration> Parse_CursorDeclaration(string name, List<string>? leadingOptions = null)
    {
        ...
        {
            declaration.CursorOptions.Add(option);
        }
    ...
    private ParseResult<SqlSetOptionStatement> ParseSetOptionStatement()
    {
        ...
        if (string.Equals(option.ResultValue.FieldName, "TRANSACTION", StringComparison.OrdinalIgnoreCase)
            && TryKeywords(["ISOLATION", "LEVEL"], out _))
        {
            return CreateParseResult(new SqlSetOptionStatement
            {
                Span = _text.CreateSpan(startSpan),
                Option = "TRANSACTION ISOLATION LEVEL",
                Value = ReadIsolationLevel()
            });
        }
    ...
    private string ReadIsolationLevel()
    {
        var first = ReadSqlIdentifier().Word;
        if (string.Equals(first, "READ", StringComparison.OrdinalIgnoreCase)
            || string.Equals(first, "REPEATABLE", StringComparison.OrdinalIgnoreCase))
        {
            return $"{first} {ReadSqlIdentifier().Word}";
        }
    ...
    private static readonly string[] SimpleKeywordStatements =
    [
        "CHECKPOINT", "RECONFIGURE", "REVERT", "SHUTDOWN"
    ];

    private ParseResult<SqlKeywordStatement> ParseKeywordStatement()
    {
        foreach (var keyword in SimpleKeywordStatements)
        {
            if (TryKeyword(keyword, out var startSpan))
            {
                var argument = string.Empty;
                if (string.Equals(keyword, "CHECKPOINT", StringComparison.OrdinalIgnoreCase) && Try(ParseNumberValue, out var checkpointArg))
                {
                    argument = checkpointArg.ResultValue.ToSql();
                }
                else if (string.Equals(keyword, "RECONFIGURE", StringComparison.OrdinalIgnoreCase) && TryKeywords(["WITH", "OVERRIDE"], out _))
                {
                    argument = "WITH OVERRIDE";
                }
    ...
    private ParseResult<SqlDbccStatement> ParseDbccStatement()
    {
        ...
        {
            return NoneResult<SqlDbccStatement>();
        }

        var command = ReadSqlIdentifier().Word;
        if (string.IsNullOrEmpty(command))
        {
            return CreateParseError("Expected DBCC command name");
        }

        var statement = new SqlDbccStatement
        {
            Span = _text.CreateSpan(startSpan),
            Command = command
        };
        ...
        if (TryMatch("(", out _))
        {
            ...
            statement.Arguments = arguments.ResultValue.Select(argument => argument.ToSql()).ToList();
            if (!TryMatch(")", out _))
            {
                return CreateParseError("Expected ) to close DBCC arguments");
            }
        }

        if (TryKeyword("WITH", out _))
        {
            statement.Options = ReadCommaSeparatedIdentifiers();
        }

        return CreateParseResult(statement);
    }

    private ParseResult<SqlBulkInsertStatement> ParseBulkInsertStatement()
    {
        if (!TryKeywords(["BULK", "INSERT"], out var startSpan))
        {
            return NoneResult<SqlBulkInsertStatement>();
        }

        var tableName = Parse_SqlIdentifier();
        if (tableName.Result == null)
        {
            return CreateParseError("Expected table name after BULK INSERT");
        }

        if (!TryKeyword("FROM", out _))
        {
            return CreateParseError("Expected FROM in BULK INSERT");
        }

        var dataFile = ParseArithmeticExpr();
        if (dataFile.HasError)
        {
            return dataFile.Error;
        }

        var statement = new SqlBulkInsertStatement
        {
            Span = _text.CreateSpan(startSpan),
            TableName = tableName.ResultValue.FieldName,
            DataFile = dataFile.ResultValue.ToSql()
        };
    ...
    private ParseResult<SqlToggleTriggerStatement> ParseToggleTriggerStatement()
    {
        var startPosition = _text.Position;
        bool enable;
        TextSpan startSpan;
        if (TryKeyword("ENABLE", out startSpan))
        {
            enable = true;
        }
        else if (TryKeyword("DISABLE", out startSpan))
        {
            enable = false;
        }
        else
        {
            return NoneResult<SqlToggleTriggerStatement>();
        }
        ...
        else
        {
            ...
            triggerNames = names.ResultValue.Select(name => name.FieldName).ToList();
        }

        if (!TryKeyword("ON", out _))
        {
            return CreateParseError("Expected ON in ENABLE/DISABLE TRIGGER");
        }
        ...
        else if (TryKeyword("DATABASE", out _))
        {
            target = "DATABASE";
        }
        else
        {
            ...
            {
                return CreateParseError("Expected target after ON in ENABLE/DISABLE TRIGGER");
            }

            target = targetName.ResultValue.FieldName;
        }

        return CreateParseResult(new SqlToggleTriggerStatement
        {
            Span = _text.CreateSpan(startSpan),
            Enable = enable,
    ...
    private ParseResult<SqlKillStatement> ParseKillStatement()
    {
        if (!TryKeyword("KILL", out var startSpan))
        {
            return NoneResult<SqlKillStatement>();
        }

        var sessionId = ParseArithmetic_Primary();
        if (sessionId.Result == null)
        {
            return CreateParseError("Expected session id after KILL");
        }

        return CreateParseResult(new SqlKillStatement
        {
            Span = _text.CreateSpan(startSpan),
            SessionId = sessionId.Result.ToSql(),
            WithStatusOnly = TryKeywords(["WITH", "STATUSONLY"], out _)
        });
    }

    private ParseResult<SqlBackupRestoreStatement> ParseBackupRestoreStatement()
    {
        bool isBackup;
        TextSpan startSpan;
        if (TryKeyword("BACKUP", out startSpan))
        {
            isBackup = true;
        }
        else if (TryKeyword("RESTORE", out startSpan))
        {
            isBackup = false;
        }
        else
        {
            return NoneResult<SqlBackupRestoreStatement>();
        }
        ...
        else if (TryKeyword("LOG", out _))
        {
            objectKind = "LOG";
        }
        else if (TryKeyword("CERTIFICATE", out _))
        {
            objectKind = "CERTIFICATE";
        }
        else if (TryKeywords(["MASTER", "KEY"], out _))
        {
            objectKind = "MASTER KEY";
        }
        else
        {
            return CreateParseError("Expected DATABASE, LOG, CERTIFICATE or MASTER KEY after BACKUP/RESTORE");
        }

        var name = string.Empty;
        if (!IsPeekKeywords("TO") && !IsPeekKeywords("FROM"))
        {
            var nameResult = Parse_SqlIdentifier();
            if (nameResult.Result == null)
            {
                return CreateParseError("Expected object name in BACKUP/RESTORE");
            }

            name = nameResult.ResultValue.FieldName;
        }

        if (isBackup ? !TryKeyword("TO", out _) : !TryKeyword("FROM", out _))
        {
            return CreateParseError("Expected TO/FROM in BACKUP/RESTORE");
        }

        var statement = new SqlBackupRestoreStatement
        {
            Span = _text.CreateSpan(startSpan),
            IsBackup = isBackup,
            ObjectKind = objectKind,
            Name = name,
            Devices = ReadAssignmentOptionList()
        };
    ...
    private ParseResult<SqlGotoStatement> ParseGotoStatement()
    {
        if (!TryKeyword("GOTO", out var startSpan))
        {
            return NoneResult<SqlGotoStatement>();
        }

        var label = Parse_SqlIdentifier();
        if (label.Result == null)
        {
            return CreateParseError("Expected label after GOTO");
        }

        return CreateParseResult(new SqlGotoStatement
        {
            Span = _text.CreateSpan(startSpan),
            Label = label.ResultValue.FieldName
        });
    }

    private ParseResult<SqlLabelStatement> ParseLabelStatement()
    {
        var startPosition = _text.Position;
        var label = Parse_SqlIdentifier();
        if (label.Result == null)
        {
            _text.Position = startPosition;
            return NoneResult<SqlLabelStatement>();
        }

        if (!TryMatch(":", out _) || IsPeekMatch(":"))
        {
            _text.Position = startPosition;
            return NoneResult<SqlLabelStatement>();
        }

        return CreateParseResult(new SqlLabelStatement
        {
            Span = _text.CreateSpan(label.ResultValue.Span),
            Label = label.ResultValue.FieldName
        });
    }
    ...
    private ParseResult<ITableSource> Parse_TableSourceWithHints()
    {
        ...
        if (tableSourceExpr is SqlTableSource systemTimeTableSource)
        {
            var forSystemTime = Parse_ForSystemTime();
            if (!string.IsNullOrEmpty(forSystemTime))
            {
                systemTimeTableSource.ForSystemTime = forSystemTime;
            }
        }
    ...
    private string Parse_ForSystemTime()
    {
        if (!TryKeywords(["FOR", "SYSTEM_TIME"], out _))
        {
            return string.Empty;
        }

        if (TryKeywords(["AS", "OF"], out _))
        {
            var asOf = ParseArithmetic_Primary();
            return $"AS OF {asOf.Result?.ToSql()}";
        }
        ...
        if (TryKeyword("FROM", out _))
        {
            var fromValue = ParseArithmetic_Primary();
            TryKeyword("TO", out _);
            var toValue = ParseArithmetic_Primary();
            return $"FROM {fromValue.Result?.ToSql()} TO {toValue.Result?.ToSql()}";
        }

        if (TryKeyword("BETWEEN", out _))
        {
            var fromValue = ParseArithmetic_Primary();
            TryKeyword("AND", out _);
            var toValue = ParseArithmetic_Primary();
            return $"BETWEEN {fromValue.Result?.ToSql()} AND {toValue.Result?.ToSql()}";
        }

        if (TryKeywords(["CONTAINED", "IN"], out _))
        {
            TryMatch("(", out _);
            var fromValue = ParseArithmetic_Primary();
            TryMatch(",", out _);
            var toValue = ParseArithmetic_Primary();
            TryMatch(")", out _);
            return $"CONTAINED IN ({fromValue.Result?.ToSql()}, {toValue.Result?.ToSql()})";
        }
    ...
    {
        var startPosition = _text.Position;
        if (TryMatch("{", out var odbcSpan))
        {
            var keyword = ReadSqlIdentifier().Word;
            var body = ParseArithmeticExpr();
            if (body.HasError)
            {
                return body.Error;
            }
            ...
            if (!TryMatch("}", out _))
            {
                return CreateParseError("Expected } to close ODBC escape sequence");
            }

            return CreateParseResult<ISqlExpression>(new SqlOdbcEscapeExpr
            {
                Span = _text.CreateSpan(odbcSpan),
                Keyword = keyword,
                Body = body.Result
            });
        }

        if (TryKeywords(["NEXT", "VALUE", "FOR"], out var nextValueSpan))
        {
            var sequenceName = Parse_SqlIdentifier();
            if (sequenceName.Result == null)
            {
                return CreateParseError("Expected sequence name after NEXT VALUE FOR");
            }

            return CreateParseResult<ISqlExpression>(new SqlNextValueForExpr
            {
                Span = _text.CreateSpan(nextValueSpan),
                SequenceName = sequenceName.ResultValue.FieldName
            });
        }
    ...
    private ParseResult<ISqlConstraint> ParseTableConstraint()
    {
        ...
        if (TryKeyword("DEFAULT", out _))
        {
            ...
            if (TryKeyword("FOR", out _))
            {
                forColumn = ReadSqlIdentifier().Word;
            }

            return new SqlConstraintDefaultValue
            {
                ConstraintName = constraintName,
                DefaultValue = defaultValue.ResultValue.ToSql(),
                ForColumn = forColumn
            };
        }
