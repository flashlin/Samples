...
測試狀態：**568 passed / 0 failed / 0 build warning**。工作區乾淨（HANDOFF.md 本身為 untracked，不需 commit）。
...
| 🎯 第四輪 30 項-C 批（30/30 完成，558→568） | _未 commit_ | **C 批(21-30)**：Service Broker 對話 DML `BEGIN DIALOG`/`END`/`MOVE`/`GET CONVERSATION GROUP`（單一 `SqlConversationStatement`，dispatch 置於 BEGIN…END 前）；通用 `CREATE {EVENT SESSION\|EVENT NOTIFICATION\|REMOTE SERVICE BINDING\|APPLICATION ROLE\|SERVER ROLE\|AVAILABILITY GROUP\|ROUTE}`（單一 `SqlCreateObjectStatement`+`CreateObjectKinds`）。dispatch ~93→~97。 |
| 🎯 第四輪 30 項-A/B 批（DROP 擴充 + ALTER 變體，20/30，538→558） | _未 commit_ | **A 批(1-10) DROP 擴充**：重做 `ParseDropStatement` 以 `SqlDropStatement.TypeName`（字串）+ `MatchMultiWordDropType` 支援 ~24 種型別（CERTIFICATE/ASSEMBLY/CREDENTIAL/AGGREGATE/RULE/DEFAULT/QUEUE/SERVICE/CONTRACT/ENDPOINT/STATISTICS、MESSAGE TYPE/MASTER KEY/SYMMETRIC KEY/PARTITION FUNCTION·SCHEME/FULLTEXT INDEX·CATALOG/SECURITY POLICY/EXTERNAL TABLE·DATA SOURCE·FILE FORMAT/COLUMN MASTER·ENCRYPTION KEY），核心型別仍走 enum。**B 批(11-20)**：通用 `ALTER <kind> [name] <action>`（`SqlAlterObjectStatement`+`AlterObjectKinds`，涵蓋 ASSEMBLY/CERTIFICATE/SERVICE/QUEUE/ENDPOINT/RESOURCE GOVERNOR/PARTITION FUNCTION·SCHEME 等）、`{ADD\|DROP} SIGNATURE`（`SqlSignatureStatement`）、`CREATE [PRIMARY] XML INDEX`（`SqlCreateIndexStatement.IsXml`/`IsPrimaryXml`）。 |
| 🎯 第三輪 30 項-C 批（goal 30/30 完成，528→538） | _未 commit_ | **C 批 21-30**：㉑ `CREATE SERVICE`（`SqlCreateServiceStatement`）㉒ `CREATE CONTRACT`（`SqlCreateContractStatement`）㉓ `CREATE MESSAGE TYPE`（`SqlCreateMessageTypeStatement`）㉔ `SEND ON CONVERSATION`（`SqlSendStatement`）㉕ `RECEIVE`（`SqlReceiveStatement`）㉖ `CREATE ENDPOINT`（`SqlCreateEndpointStatement`）㉗㉘ `CREATE {WORKLOAD GROUP\|RESOURCE POOL}`（`SqlCreateResourceGovernorObjectStatement`+Kind）㉙ `CREATE COLUMN {MASTER\|ENCRYPTION} KEY`（`SqlCreateColumnKeyStatement`）㉚ `READTEXT`/`WRITETEXT`/`UPDATETEXT`（`SqlTextPointerStatement`）。順手把 `ReadParenthesizedAssignmentList` 的值讀取改用 `ParseArithmetic_Primary`（支援數值/字串值，修 ENDPOINT `LISTENER_PORT = 5022`）。dispatch ~77→~93。 |
| 🎯 第三輪 30 項-B 批（goal 30 項，B 批 11-20 完成，517→528） | _未 commit_ | **B 批 11-20**：⑪⑫ `ALTER DATABASE {ADD\|ADD LOG\|MODIFY} FILE (spec)`（`SqlAlterDatabaseStatement.FileAction`/`FileSpec`）⑬ `ALTER TABLE {ADD\|DROP} PERIOD FOR SYSTEM_TIME`（`SqlAlterTablePeriod`）⑭⑮ DDL trigger `CREATE TRIGGER … ON {DATABASE\|ALL SERVER} FOR <ddl_event>`（`SqlCreateTriggerStatement.DdlEvents`）⑯ `CREATE ASSEMBLY`（`SqlCreateAssemblyStatement`）⑰⑱ `CREATE {RULE\|DEFAULT} name AS expr`（`SqlCreateRuleOrDefaultStatement`）⑲ `CREATE AGGREGATE`（`SqlCreateAggregateStatement`）⑳ `CREATE QUEUE`（`SqlCreateQueueStatement`）。 |
| 🎯 第三輪 30 項-A 批（goal review 未列出 item，A 批 1-10 完成，504→517） | _未 commit_ | 兩輪 triage（~70 候選）確認常見語法皆可解析、找出真正缺者。**A 批 1-10**：① `LIKE … ESCAPE 'c'`（`SqlConditionExpression.Escape`，原 ESCAPE 被丟棄）② `SELECT @v = expr` 變數指派（既有 `SqlAssignExpr`，補測試）③ `IS [NOT] DISTINCT FROM`（`ComparisonOperator` 新 2 員，多字運算子排在 `IS` 前）④ 量化比較 `op {ALL\|ANY\|SOME} (subquery)`（`SqlQuantifiedExpr`；原誤解析為函式 `ALL(SELECT` 截斷）⑤ `N'unicode'`（既有保留，補測試）⑥⑦⑧ `CREATE EXTERNAL {DATA SOURCE\|FILE FORMAT\|TABLE}`（單一 `SqlCreateExternalStatement`+Kind）⑨ `CREATE SECURITY POLICY … ADD FILTER/BLOCK PREDICATE … ON …`（`SqlCreateSecurityPolicyStatement`）⑩ `CREATE [DATABASE SCOPED] CREDENTIAL`（`SqlCreateCredentialStatement`，dispatch 置 CREATE DATABASE 前）。 |
| 🎯 拉高完成度批次三-C 批（goal 30/30 完成，490→504） | _未 commit_ | **C 批 21-30**：㉑ `CREATE FULLTEXT CATALOG`（`SqlCreateFulltextCatalogStatement`）㉒ `CREATE FULLTEXT STOPLIST`（`SqlCreateFulltextStoplistStatement`）㉓ `CREATE SYMMETRIC KEY`（`SqlCreateSymmetricKeyStatement`）㉔ `{OPEN\|CLOSE} [ALL] SYMMETRIC KEY[S]`（`SqlSymmetricKeyStatement`，dispatch 置於游標 OPEN/CLOSE 前）㉕ `ALTER FULLTEXT INDEX ON t <action>`（`SqlAlterFulltextIndexStatement`）㉖ `ALTER SERVER CONFIGURATION SET ...`（`SqlAlterServerConfigurationStatement`）㉗ CTAS `CREATE TABLE t AS SELECT`（`SqlCreateTableExpression.AsSelect`）㉘ `WITH XMLNAMESPACES (...) <select>`（`SqlXmlNamespacesStatement`，URI 用 `ParseSqlQuotedString` 免吃 `AS`）㉙ `CHECKPOINT n`/`RECONFIGURE WITH OVERRIDE`（`SqlKeywordStatement.Argument`）㉚ `OPENROWSET(BULK ...)`（`Parse_FunctionArgument` 處理 `BULK`；**修復 `Parse_FromTableSources` 空清單 `[0]` 崩潰**）。dispatch ~59→~77。 |
| 🎯 拉高完成度批次三-B 批（goal 30 項，B 批 11-20 完成，476→490） | _未 commit_ | **B 批 11-20**：⑪ `ALTER INDEX … SET (opt=val)`（`SqlAlterIndexStatement.Options`）⑫ `CREATE TABLE … ON filegroup\|ps(col) / TEXTIMAGE_ON / WITH (opt)`（`OnFileGroup`/`TextImageOn`/`WithOptions`，`Parse_CreateTableOptions`）⑬ 時間性 `PERIOD FOR SYSTEM_TIME (c1,c2)`（`Period`）⑭ `EXEC (...) AT linked_server`（`AtLinkedServer`）⑮ `JSON_OBJECT('k': v)` 冒號鍵值對（`Parse_FunctionArgument` 用 `:` 組 `SqlAssignExpr`）⑯ `CREATE CERTIFICATE`（`SqlCreateCertificateStatement`）⑰ `CREATE MASTER KEY`（`SqlCreateMasterKeyStatement`）⑱ `SETUSER`（`SqlSetUserStatement`）⑲ `CREATE SPATIAL INDEX`（`IsSpatial`）⑳ `BACKUP/RESTORE CERTIFICATE\|MASTER KEY`（擴 `SqlBackupRestoreStatement`，MASTER KEY 無名稱） |
| 🎯 拉高完成度批次三（goal：30 項，A 批 1-10 完成，461→476） | _未 commit_ | 先用暫時 triage 測試批次探查 ~60 候選語法的解析狀態（ERR/EXC/ok），據此挑「真正缺且高頻」者。**A 批 1-10**：① GRANT/REVOKE/DENY 欄位層級 `(col1, col2)`（`SqlPermissionStatement.Columns`）② `EXEC @ret = proc args` 回傳值擷取（`ReturnVariable`，原本擲例外）③ `DECLARE @t TABLE` inline table 約束（`TableConstraints`，新 `Parse_ParenthesizedTableElements`）④ `ALTER TABLE ADD CONSTRAINT df DEFAULT val FOR col`（`SqlConstraintDefaultValue.ForColumn`）⑤ 獨立 `ENABLE/DISABLE TRIGGER … ON {table\|DATABASE\|ALL SERVER}`（`SqlToggleTriggerStatement`）⑥ `ALTER TABLE REBUILD [WITH(...)]`（`SqlAlterTableRebuild`）⑦ `CREATE [CLUSTERED\|NONCLUSTERED] COLUMNSTORE INDEX`（`SqlCreateIndexStatement.IsColumnstore`，欄位可省）⑧ `ALTER AUTHORIZATION ON [class::]obj TO principal`（`SqlAlterAuthorizationStatement`）⑨ `ALTER TABLE SET (option)`（`SqlAlterTableSet`）⑩ `ALTER TABLE SWITCH [PARTITION n] TO target [PARTITION m]`（`SqlAlterTableSwitch`） |
...
**完成度**（第三輪 30 項 goal **30/30 完成**）：本輪先 review todo「未列出」的 T-SQL，補上並實作了大量先前未涵蓋語法 —— 高頻運算式/述詞（`LIKE ESCAPE`、量化比較 `ALL/ANY/SOME`、`IS [NOT] DISTINCT FROM`、變數指派 SELECT、`N'…'`）、PolyBase（EXTERNAL TABLE/DATA SOURCE/FILE FORMAT）、安全性（SECURITY POLICY、CREDENTIAL、CERTIFICATE/MASTER KEY/SYMMETRIC KEY、COLUMN MASTER/ENCRYPTION KEY）、DB 檔案（ALTER DATABASE ADD/MODIFY FILE）、temporal（ALTER TABLE ADD/DROP PERIOD）、DDL 觸發器（ON DATABASE/ALL SERVER）、CLR（ASSEMBLY/AGGREGATE/RULE/DEFAULT）、Service Broker（QUEUE/SERVICE/CONTRACT/MESSAGE TYPE/SEND/RECEIVE）、Resource Governor、ENDPOINT、READTEXT/WRITETEXT/UPDATETEXT。dispatch 頂層語句 ~77→~93。`[ ]` 僅剩 `ALTER COLUMN ADD/DROP` 子選項（罕見）。
