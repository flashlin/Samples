...
測試狀態：**352 passed / 0 failed / 0 build warning**。工作區乾淨（HANDOFF.md 本身為 untracked，不需 commit）。
...
**立即動作建議**：DML（含 EXEC 動態 SQL/具名參數、MERGE `OUTPUT $action`）+ 完整 DDL + 控制流程（含 `DECLARE @t TABLE`）+ 批次 + 權限皆已備齊，T-SQL 主要語句已大致覆蓋。剩下多為選項缺口 / 罕用語句：`ALTER VIEW/PROC/FUNCTION/TRIGGER`、TRIGGER `WITH`、FUNCTION `WITH SCHEMABINDING`、權限 `object_type::` 前綴、`DECLARE @c CURSOR`、`BEGIN DISTRIBUTED TRANSACTION`、`EXEC (...) AT linked_server`、SELECT 進階（具名 WINDOW 延伸，見 §4）。建議與使用者確認下一個重點再續。
