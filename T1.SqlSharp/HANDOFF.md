...
測試狀態：**378 passed / 0 failed / 0 build warning**。工作區乾淨（HANDOFF.md 本身為 untracked，不需 commit）。
...
1. 🟢 **DDL 續攻**（DROP/TRUNCATE/ALTER TABLE（含第二階段）/CREATE VIEW/INDEX/PROCEDURE/FUNCTION（含 multi-statement TVF）/TRIGGER/SCHEMA/DATABASE、GRANT/REVOKE/DENY 已完成）：剩 ADD 混合欄位+約束、`ALTER VIEW/PROC/FUNCTION/TRIGGER`、DATABASE/SCHEMA 進階選項。
...
**立即動作建議**：DML（含 EXEC 動態 SQL/具名參數、MERGE `OUTPUT $action`）+ 完整 DDL + 控制流程（含 `DECLARE @t TABLE`）+ 批次 + 權限皆已備齊，T-SQL 主要語句已大致覆蓋。剩下多為罕用語句 / 進階選項：權限 `object_type::` 前綴、`BEGIN DISTRIBUTED TRANSACTION`、`EXEC (...) AT linked_server`、`CREATE SCHEMA`/`DATABASE` 進階選項（`ON`/`LOG ON`/`COLLATE`）、CURSOR 進階選項（SCROLL/STATIC/GLOBAL）、SELECT 進階（具名 WINDOW 延伸，見 §4）、SET 取值型（`SET ROWCOUNT n`/`TRANSACTION ISOLATION LEVEL`）、`WAITFOR`/`GOTO`/標籤、運維語句（`BULK INSERT`/`BACKUP`/`DBCC`/`KILL`）。建議與使用者確認下一個重點再續。
