...
測試狀態：**419 passed / 0 failed / 0 build warning**。工作區乾淨（HANDOFF.md 本身為 untracked，不需 commit）。
...
**完成度**：依語法項目加權（`[x]`=1、`[~]`=0.5，共 191 項）≈ **89.8%**（未完成 ≈10.2%）；若「部分支援」算可用則 ≈ **96.9%**。以實際使用頻率看覆蓋更高，剩餘幾乎都是罕用/運維/邊緣語法。

**立即動作建議**：T-SQL 主要語句 + 常見選項已大致覆蓋。剩餘 `[ ]`（6 項，皆低頻）：時間性 `FOR SYSTEM_TIME`、運維語句（`BULK INSERT`/`BACKUP`/`RESTORE`/`DBCC`/`KILL`）、`CREATE PARTITION FUNCTION/SCHEME` 等罕用 DDL、ODBC 跳脫 `{ fn … }`、`$PARTITION`/`$IDENTITY`、`ALTER COLUMN ADD/DROP` 子選項。`[~]` 多為單一語句的進階選項缺口（如 `EXEC ... AT linked_server`、`CREATE DATABASE ON/LOG ON`、具名 WINDOW 行內延伸）。建議與使用者確認下一個重點再續。
