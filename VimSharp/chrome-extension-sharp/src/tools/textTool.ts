...
export function convertCsvToDataTable(text: string, delimiter: string = '\t'): DataTable {
  ...
  // 使用重構後的函數來取得欄位定義
  const columns = getCsvDataTableColumnsByPapaParseResult(result);
...
/**
 * 根據 Papa.parse 結果推測每個欄位的資料型別
 * @param result Papa.parse 的解析結果
 * @returns DataTableColumn 陣列，包含欄位名稱和型別
 */
export function getCsvDataTableColumnsByPapaParseResult(result: PapaParseResult): DataTableColumn[] {
  if (!result.data || result.data.length === 0) {
    return [];
  }
...
export function getCsvDataTableColumns(csvText: string, delimiter: string = '\t'): DataTableColumn[] {
  ...
  return getCsvDataTableColumnsByPapaParseResult(result);
}
