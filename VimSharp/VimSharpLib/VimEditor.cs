...
public class VimEditor
{
    ...
    private void HandleVerticalScrollAndUpdateCursor(int targetCursorY, int scrollDirection)
    {
        ...
        // 計算可見區域的有效高度（考慮狀態欄）
        int effectiveViewPortHeight =
            Context.IsStatusBarVisible ? Context.ViewPort.Height - 1 : Context.ViewPort.Height;
    ...
    private void RenderStatusBar(ColoredChar[,] screenBuffer)
    {
        ...
        {
            if (Context.ViewPort.X + i >= 0 && Context.ViewPort.X + i < bufferWidth &&
                i < Context.StatusBar.Chars.Length)
            {
