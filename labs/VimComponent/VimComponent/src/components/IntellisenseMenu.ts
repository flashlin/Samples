...
export class IntellisenseMenu {
  ...
  show(items: IntellisenseItem[], x: number, y: number, parentElement: HTMLElement, initialFilterText: string = ''): void {
    this.hide();
    ...
    this.currentFilterText = initialFilterText;
    ...
    items.forEach((item, index) => {
      ...
      textElement.innerHTML = this.highlightMatches(item.text, this.currentFilterText);
  ...
  private injectHighlightStyles(): void {
    if (document.querySelector('#intellisense-highlight-style')) {
      return;
    }
    
    const style = document.createElement('style');
    style.id = 'intellisense-highlight-style';
    style.textContent = `
      .intellisense-text .match-highlight {
        color: #ffeb3b;
        font-weight: bold;
      ...
      .intellisense-item.selected .match-highlight {
        color: #fff;
  ...
  private highlightMatches(text: string, filterText: string): string {
    if (!filterText) {
    ...
    for (let i = 0; i < matchPositions.length; i++) {
      ...
      if (pos > lastIndex) {
        result += this.escapeHtml(text.substring(lastIndex, pos));
      }
      
      const isConsecutive = i < matchPositions.length - 1 && matchPositions[i + 1] === pos + 1;
      
      if (isConsecutive) {
        let endPos = pos;
        while (i < matchPositions.length - 1 && matchPositions[i + 1] === endPos + 1) {
          endPos++;
          i++;
        }
        result += `<span class="match-highlight">${this.escapeHtml(text.substring(pos, endPos + 1))}</span>`;
        lastIndex = endPos + 1;
      } else {
        result += `<span class="match-highlight">${this.escapeHtml(text[pos])}</span>`;
        lastIndex = pos + 1;
      }
    }
