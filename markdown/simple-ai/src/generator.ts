export function convertIdToNumbersText(id: string) {
  const first: any = {
    'A': 10,
    'B': 11,
    'C': 12,
...
export function generateRandomID() {
  ...
  let idNumbers = convertIdToNumbersText(str);
...
export function convertId9ToNumbers(id: string): number[] {
  const idStr = id.substring(0, 9);
  const numbersText = convertIdToNumbersText(idStr);

  //console.log(`${idStr} => ${numbersText}`);

  const numbers = [];
  for (let n = 0; n < numbersText.length; n++) {
    numbers.push(numbersText[n].charCodeAt(0) - '0'.charCodeAt(0));
  }
  return numbers;
}
