...
class LitSeq2Seq(pl.LightningModule):
    def __init__(self, input_dim=LINQ_VOCAB_SIZE, output_dim=TSQL_VOCAB_SIZE,
                 hid_dim=256,
                 enc_layers=3, enc_heads=8, enc_pf_dim=512, enc_dropout=0.1,
                 dec_layers=3, dec_heads=8, dec_pf_dim=512, dec_dropout=0.1,
                 device=None):
        super().__init__()
        if device is None:
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.lr_scheduler = None
        self.encoder = enc = Encoder(input_dim, hid_dim, enc_layers, enc_heads, enc_pf_dim, enc_dropout, device)
        self.decoder = dec = Decoder(output_dim, hid_dim, dec_layers, dec_heads, dec_pf_dim, dec_dropout, device)
        src_pad_idx = 0
        trg_pad_idx = 0
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        y_hat = y_hat.contiguous().view(-1, output_dim)
